...
def get_ft_user():
    return {
        "username":
        os.environ.get("NEXTLINUX_FUNCTIONAL_TEST__ACCOUNT_USER", "ft_user"),
        "password":
        os.environ.get("NEXTLINUX_FUNCTIONAL_TEST_ACCOUNT_PASSWORD", "f00b4r"),
    }
...
def get_ft_user_api_conf():
    ...
    DEFAULT_API_CONF, ex:
        {
            'NEXTLINUX_API_USER': '<username>',
            'NEXTLINUX_API_PASS': '<password>',
            'NEXTLINUX_BASE_URL': '<base_url>',
            'NEXTLINUX_API_ACCOUNT': '<account_name>'
        }
    ...
    api_conf["NEXTLINUX_API_USER"] = ft_user["username"]
    api_conf["NEXTLINUX_API_PASS"] = ft_user["password"]
    api_conf["NEXTLINUX_API_ACCOUNT"] = FT_ACCOUNT
...
def create_functional_test_account_with_teardown(request):
    ...
    create_resp = http_post(["accounts"], {
        "name": FT_ACCOUNT,
        "email": "admin@next-linux.systems"
    })
...
USER_API_CONFS = [
    ...
    pytest.param(get_ft_user_api_conf,
                 id="functional_test_account_fullcontrol_user"),
]
