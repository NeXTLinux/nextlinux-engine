...
class TestMavenVersionHandling(unittest.TestCase):
    ...
    def _check_ordering_(self, versions, increasing=True):
        ...
        for l, r in zip(range(len(mvs)), range(1, len(mvs))):
            ...
            self.assertLess(
                small,
                big,
                "Expected {} to be less than {}".format(small.value, big.value),
            )
            self.assertEqual(
    ...
    def _check_op_(self, v1, v2, op):
        ...
        if op == "==":
            self.assertEqual(
                mv1, mv2, "Expected {} and {} to be equal".format(mv1.value, mv2.value)
            )
            self.assertEqual(
                mv1.compare_to(mv2),
                0,
            ...
            self.assertEqual(
                mv1.__hash__(), mv2.__hash__(), "Expected hash code to be equal"
            )
        elif op == ">":
            self.assertGreater(
                mv1,
                mv2,
                "Expected {} to be greater than {}".format(mv1.value, mv2.value),
            )
            self.assertLess(
                mv2,
                mv1,
            ...
            self.assertGreater(
                mv2, mv1, "Expected {} to be less than {}".format(mv1.value, mv2.value)
            )
            self.assertLess(
                mv1, mv2, "Expected {} to be less than {}".format(mv1.value, mv2.value)
            )
            self.assertEqual(
                mv1.compare_to(mv2),
                -1,
