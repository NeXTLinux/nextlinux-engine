...
def test_domain_model_positive():
    positive_check([{"name": "somedomain"}, {"name": ""}], Domain)
    return True


def test_domain_model_negative():
    negative_check(
        [{"name": "somedomain", "badkey": "badvalue"}, {"n": ""}, {}], Domain
    )
    return True


def test_principal_model_positive():
    positive_check([{"name": "somedomain"}, {"name": ""}], Principal)
    return True


def test_principal_model_negative():
    negative_check(
        [{"name": "somedomain", "badkey": "badvalue"}, {"n": ""}, {}], Principal
    )
    return True


def test_action_model_positive():
    positive_check([{"domain": "somedomain", "action": "act1", "target": "t1"}], Action)
    return True


def test_action_model_negative():
    negative_check([{}, {"domain": ""}, {"blah": "", "domain": "blah2"}], Action)
    return True
...
def test_authorizationdecision_model_positive():
    positive_check(
        [
            ...
            {"principal": {"name": "user"}, "allowed": [], "denied": [], "ttl": 10},
        ],
        AuthorizationDecision,
    )
    return True
...
def test_authorizationdecision_model_negative():
    negative_check(
        [
            {
                "principal": {},
                "allowed": [{"domain": "d", "action": "a", "target": "t"}],
                "denied": [{"domain": "d", "action": "a", "target": "t"}],
                "ttl": 10,
            },
            {
                "principal": {"name": "user"},
            ...
            {"principal": {"name": "user"}, "allowed": [], "denied": []},
        ],
        AuthorizationDecision,
    )
    return True
