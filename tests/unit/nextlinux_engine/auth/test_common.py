...
_record_ecr_unavailable = {
    ...
    "record_state_key": "inactive",
    "record_state_val":
    time.time(),  # note: technically this could yield nondeterministic results
    "registry_type": "awsecr",
    "registry_meta": json.dumps(_test_registry_meta),
    "registry_verify": True,
}
...
def test_get_creds_by_registry_unavailable():
    with pytest.raises(Exception):
        get_creds_by_registry("docker.io", "library/node",
                              [_record_ecr_unavailable])
...
def test_registry_record_matches_exact(registry_record_str, registry,
                                       repository):
...
def test_registry_record_matches_wildcard(registry_record_str, registry,
                                          repository):
    assert registry_record_matches(registry_record_str, registry, repository)
...
def test_registry_record_matches_non(registry_record_str, registry,
                                     repository):
    assert not registry_record_matches(registry_record_str, registry,
                                       repository)
