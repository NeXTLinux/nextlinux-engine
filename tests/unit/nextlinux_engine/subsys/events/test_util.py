...
from nextlinux_engine.subsys.taskstate import base_state, complete_state
...
def test_analysis_complete_notification_factory():
    ...
    assert result.details["curr_eval"]["analysis_status"] == complete_state(
        "analyze")
    assert result.details["curr_eval"]["imageDigest"] == IMAGE_DIGEST
    assert result.details["last_eval"]["analysis_status"] == base_state(
        "analyze")
