...
def test_init_state(param):
    actual_state = init_state(param["state_type"], param["current_state"],
                              param["reset"])
...
def test_working_state(state_type):
    assert working_state(
        state_type) == state_graphs[state_type]["working_state"]
...
def test_next_state(state_type, current):
    if not current:
        assert (next_state(
            state_type,
            current) == state_graphs[state_type]["transitions"]["init"])
    else:
        assert (next_state(
            state_type,
            current) == state_graphs[state_type]["transitions"][current])
...
def test_complete_state(state_type):
    assert complete_state(
        state_type) == state_graphs[state_type]["complete_state"]
...
def test_orphaned_state(state_type):
    if "orphaned_state" in state_graphs[state_type]:
        assert orphaned_state(
            state_type) == state_graphs[state_type]["orphaned_state"]
    else:
        assert orphaned_state(
            state_type) == state_graphs[state_type]["fault_state"]
