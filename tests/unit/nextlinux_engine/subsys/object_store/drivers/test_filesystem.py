...
class TestFilesystemObjectStorageDriver:
    ...
    def test_uri_for(self, param):
        ...
        expected_path = "file://{}".format(
            driver._get_archive_filepath(param["userId"], param["bucket"],
                                         param["key"]))
        assert (driver.uri_for(param["userId"], param["bucket"],
                               param["key"]) == expected_path)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                {
                    "uri": "/usr/local/bin/grype",
                    "expected": "/usr/local/bin/grype"
                },
                id="unix-path",
            ),
            pytest.param(
                {
                    "uri": "C:\\Documents\\UnitTest",
                    "expected": "\\Documents\\UnitTest"
                },
                id="windows-path",
            ),
            pytest.param(
                {
                    "uri": "https://www.next-linux.systems",
                    "expected": ""
                },
                id="url"),
        ],
    ...
    def test_get_archive_filepath(self, param):
        driver = FilesystemObjectStorageDriver(
            config={"archive_data_dir": self.__archive_data_dir__})
        actual_fp = driver._get_archive_filepath(param["userId"],
                                                 param["bucket"], param["key"])
        ...
        expected_filehash = hashlib.new("md5",
                                        param["key"].encode("utf8"),
