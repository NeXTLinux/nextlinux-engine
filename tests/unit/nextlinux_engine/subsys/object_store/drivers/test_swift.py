...
class TestSwiftObjectStorageDriver:
    ...
    def test_parse_uri(self, param):
        assert (SwiftObjectStorageDriver({
            "unittest": True
        })._parse_uri(param["uri"]) == param["expected"])
            ...
            pytest.param(
                {
                    ...
                    "dakakey",
                    "expected":
                    "swift://dakacontainername/dakaneye/dakabucket/dakakey",
                },
                id="success",
            )
        ],
    ...
    def test_uri_for(self, param):
        assert (SwiftObjectStorageDriver({
            "unittest": True,
            "container": "dakacontainername"
        }).uri_for(param["userId"], param["bucket"],
