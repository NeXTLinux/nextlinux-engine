...
class TestLegacyDbDriver:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                {
                    "uri":
                    "postgresql://other@localhost/bucket/key?connect_timeout=10&application_name=myapp",
            ...
            pytest.param(
                {
                    "uri":
                    "postgresql://other@localhost?connect_timeout=10&application_name=myapp",
                    "expected": ValueError(),
                },
                id="expected-failure",
            ),
    ...
    def test_uri_for(self, param):
        assert (LegacyDbDriver({}).uri_for(param["userId"], param["bucket"],
                                           param["key"]) == param["expected"])
            ...
            pytest.param(
                {
                    ...
                    b"\x80abc",
                    "expected_encoding":
                    str(base64.encodebytes(b"\x80abc"), "utf-8"),
                    "expected_b64":
                    True,
                },
            ...
            pytest.param(
                {
                    "input": {
                        "b64_encoded": True,
                        "jsondata": str(base64.encodebytes(b"\x80abc"),
                                        "utf-8"),
                    },
...
class TestDbDriver:
    ...
    def test_uri_for(self, param):
        assert (DbDriver({}).uri_for(param["userId"], param["bucket"],
                                     param["key"]) == param["expected"])
