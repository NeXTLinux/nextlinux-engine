...
from nextlinux_engine.services.policy_engine.engine.policy.gates import secrets
...
def test_secret_content_trigger(image, exec_context, secrets_gate,
                                test_context):
    secret_content_trigger = secrets.SecretContentChecksTrigger(
        parent_gate_cls=secrets_gate.__class__,
    ...
    if test_context["expected_fire"]:
        ...
        assert set(test_context["expected_msgs"]) == {
            fired.msg
            for fired in secret_content_trigger.fired
        }
    else:
