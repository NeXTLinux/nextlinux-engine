...
from nextlinux_engine.services.policy_engine.engine.policy.gates import files
...
file_attribute_match_trigger_tests = [
    {
        "trigger_params": {
            ...
            "checksum":
            "5a05c656df50a3d9fb6a299716196a28dd33dc55d154fd72d9bab89a5e1815ab",
    ...
    },
    {
        ...
        "expected_fire": True,
        "expected_msgs":
        ["filename=/usr/local/test_file and skip_missing=False"],
    },
]


@pytest.mark.parametrize("test_context", file_attribute_match_trigger_tests)
def test_file_attribute_match_trigger(files_gate, exec_context, image,
                                      test_context):
...
def test_suid_check_trigger(files_gate, exec_context, image):
    ...
    suid_check_trigger = files.SuidCheckTrigger(
        parent_gate_cls=files_gate.__class__)
...
content_match_trigger_tests = [
    ...
    },
    {
        "trigger_params": {
            "regex_name":
            "(?i).*aws_access_key_id( *=+ *).*(?<![A-Z0-9])[A-Z0-9]{20}(?![A-Z0-9]).*"
        },
        "expected_fire":
        True,
        "expected_msgs": [
