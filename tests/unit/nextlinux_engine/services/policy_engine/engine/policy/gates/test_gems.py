...
from nextlinux_engine.db.entities.policy_engine import GemMetadata, Image, ImagePackage
from nextlinux_engine.services.policy_engine.engine.policy.gate import ExecutionContext
from nextlinux_engine.services.policy_engine.engine.policy.gates import gems
...
def test_not_latest_trigger(gems_gate, exec_context, image):
    ...
    not_latest_trigger = gems.NotLatestTrigger(
        parent_gate_cls=gems_gate.__class__)
...
def test_not_official_trigger(gems_gate, exec_context, image):
    ...
    not_official_trigger = gems.NotOfficialTrigger(
        parent_gate_cls=gems_gate.__class__)
    gems_gate.prepare_context(image, exec_context)
...
def test_bad_version_trigger(gems_gate, exec_context, image):
    ...
    bad_version_trigger = gems.BadVersionTrigger(
        parent_gate_cls=gems_gate.__class__)
    gems_gate.prepare_context(image, exec_context)
...
def test_blacklisted_gem_trigger(gems_gate, exec_context, image, test_context):
    ...
    if test_context["expected_fire"]:
        assert assert_fired_with_msgs(blacklisted_gem_trigger,
                                      test_context["expected_msgs"])
    else:
