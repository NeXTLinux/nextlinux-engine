...
from nextlinux_engine.db.db_grype_db_feed_metadata import NoActiveGovulnersDB
...
class TestGovulnersDBSyncTask:
    ...
    @pytest.fixture
    def mock_calls_for_sync(self, mock_query_active_dbs_with_data,
                            mock_get_local_grypedb_checksum):
        """
    ...
    def setup_for_thread_testing(self, mock_calls_for_sync, monkeypatch):
        ...
        mock_calls_for_sync(
            GovulnersDBFeedMetadata(archive_checksum=old_checksum), "")

        # Mock the update_grypedb method for task to sleep and update mocks for active and local grype dbs
        def _mock_update_grypedb_for_thread1(active_grypedb=None,
                                             grypedb_file_path=None):
            ...
            mock_calls_for_sync(
                GovulnersDBFeedMetadata(archive_checksum=new_checksum),
                new_checksum)

        monkeypatch.setattr(GovulnersDBSyncManager, "_update_grypedb",
                            _mock_update_grypedb_for_thread1)
    ...
    def test_no_active_grypedb(self, monkeypatch):
        ...
        with pytest.raises(NoActiveDBSyncError):
            GovulnersDBSyncManager.run_grypedb_sync(Mock())
    ...
    def test_matching_checksums(self, mock_calls_for_sync):
        ...
        sync_ran = GovulnersDBSyncManager.run_grypedb_sync(Mock())
    ...
    def test_mismatch_checksum(self, mock_calls_for_sync, monkeypatch):
        ...
        mock_calls_for_sync(
            mock_active_db=GovulnersDBFeedMetadata(
        ...
        sync_ran = GovulnersDBSyncManager.run_grypedb_sync(
            Mock(), grypedb_file_path="test/bypass/catalog.txt")

        assert sync_ran is True

    def test_uninitialized_grype_wrapper(self):
        assert GovulnersDBSyncManager._get_local_grypedb_checksum() is None
    ...
    def test_lock_across_threads(self, setup_for_thread_testing):
        ...
        with ThreadPoolExecutor() as executor:
            # run thread1
            thread1 = executor.submit(GovulnersDBSyncManager.run_grypedb_sync,
            ...
            for attempt in range(10):
                if GovulnersDBSyncLock._lock.locked():
                    lock_acquired = True
                    synchronous_task = GovulnersDBSyncManager.run_grypedb_sync(
                        Mock(), grypedb_file_path="test/bypass/catalog.txt")
                    break
                else:
                    time.sleep(1)
            ...
            for attempt in range(5):
                if GovulnersDBSyncLock._lock.locked():
                    ...
                    with pytest.raises(GovulnersDBSyncLockAquisitionTimeout):
                        GovulnersDBSyncManager.run_grypedb_sync(
                            Mock(),
