...
def test_get_namespace_related_names():
    ...
    vulnerabilities.ThreadLocalFeedGroupNameCache.add([("centos:8", True),
                                                       ("rhel:8", True)])
    # When centos feed updates
    assert set(
        vulnerabilities.get_namespace_related_names(
            "centos", "8", mapped_to_centos)) == {"centos", "fedora"}

    # When rhel feed updates
    assert set(
        vulnerabilities.get_namespace_related_names(
            "rhel", "8", mapped_to_rhel)) == {"rhel"}
    ...
    vulnerabilities.ThreadLocalFeedGroupNameCache.add([("centos:7", False),
                                                       ("rhel:7", True)])

    assert (set(
        vulnerabilities.get_namespace_related_names(
            "centos", "7", mapped_to_centos)) == set())
    assert set(
        vulnerabilities.get_namespace_related_names(
            "rhel", "7", mapped_to_rhel)) == {"rhel", "centos", "fedora"}
    ...
    mapped_to_centos = []
    mapped_to_rhel = ["rhel", "centos", "fedora"]
    vulnerabilities.ThreadLocalFeedGroupNameCache.add([("centos:7", True),
                                                       ("rhel:7", True)])

    assert set(
        vulnerabilities.get_namespace_related_names(
            "centos", "7", mapped_to_centos)) == {"centos"}
    assert set(
        vulnerabilities.get_namespace_related_names(
            "rhel", "7", mapped_to_rhel)) == {"rhel", "fedora"}
