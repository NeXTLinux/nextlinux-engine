...
class TestVulnerabilityIdentity:
    ...
    def test_from_with_nvd(self, test_input):
        ...
        assert isinstance(identity_objects,
                          list) and len(identity_objects) == len(test_input)
    ...
    def test_from_without_nvd(self):
        ...
        assert identity_objects
        assert isinstance(identity_objects,
                          list) and len(identity_objects) == 1
    ...
    def test_equality_constant_artifact(self, lhs, rhs, expected):
        ...
        assert (VulnerabilityIdentity.from_match(lhs) ==
                VulnerabilityIdentity.from_match(rhs)) == expected
...
class TestRankedVulnerabilityMatch:
    ...
    def test_from(self):
        ...
        ranked_match = RankedVulnerabilityMatch.from_match(
            match, FeedGroupRank())
    ...
    def test_equality_constant_artifact(self, lhs, rhs, expected):
        ...
        assert (RankedVulnerabilityMatch.from_match(
            lhs, FeedGroupRank()) == RankedVulnerabilityMatch.from_match(
                rhs, FeedGroupRank())) == expected
...
class TestImageVulnerabilitiesDeduplicator:
    ...
    def test_execute(self, test_input, expected_index):
        ...
        results = ImageVulnerabilitiesDeduplicator(
            FeedGroupRank()).execute(test_input)
    ...
    def test_execute_absolute_duplicates(self, count):
        ...
        results = ImageVulnerabilitiesDeduplicator(
            FeedGroupRank()).execute(input_matches)
        assert len(results) == 1
    ...
    def test_execute_invalid_input(self, test_input):
        assert (ImageVulnerabilitiesDeduplicator(
...
class TestTimestampMerger:
    ...
    def test_transfer_vulnerability_timestamps_invalid_input(
        ...
        assert (transfer_vulnerability_timestamps(
            source=test_source, destination=test_destination) == expected)
    ...
    def test_transfer_vulnerability_timestamps_single(self, test_source,
                                                      test_destination):
        ...
        results = transfer_vulnerability_timestamps(source=[source],
    ...
    def test_transfer_vulnerability_timestamps_multiple(self):
        ...
        for result in results:
            if (result.vulnerability.vulnerability_id ==
