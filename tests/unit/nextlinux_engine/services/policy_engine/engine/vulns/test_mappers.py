...
class TestJavaMapper:
    ...
    def test_image_content_to_grype_metadata(self, input_metadata,
                                             expected_output):
        # Function under test
        ...
        [
            ...
            pytest.param(
                ENGINE_PACKAGE_MAPPERS.get("java"),
                {
                    ...
                    "N/A",
                    "location":
                    "/usr/share/logstash/vendor/bundle/jruby/2.5.0/gems/march_hare-4.3.0-java/lib/ext/rabbitmq-client.jar:amqp-client",
                    "maven-version":
                    "5.9.0",
                    "metadata": {
                        "pom.properties":
            ...
            pytest.param(
                ...
                {
                    "cpes":
                    ["cpe:2.3:a:bsdutils:bsdutils:1:2.33.1-0.1:*:*:*:*:*:*:*"],
                    "license":
                    "BSD-2-clause BSD-3-clause BSD-4-clause GPL-2 GPL-2+ GPL-3 GPL-3+ LGPL LGPL-2 LGPL-2+ LGPL-2.1 LGPL-2.1+ LGPL-3 LGPL-3+ MIT public-domain",
                    "licenses": [
                        "BSD-2-clause",
                    ],
                    "origin":
                    "LaMont Jones <lamont@debian.org> (maintainer)",
                    "package":
                    "bsdutils",
                    "size":
                    "293000",
...
class TestVulnerabilityMapper:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "foobar", "vulndb:xyz", None, "foobar", id="none-nvd-refs"),
            pytest.param(None, None, None, None, id="all-none"),
            pytest.param(
                "foobar", "vulndb:xyz", [], "foobar", id="no-nvd-refs"),
            pytest.param(
                "foobar", "vulndb:xyz", ["1"], "foobar",
                id="invalid-nvd-refs"),
            pytest.param("foobar", None, [], "foobar", id="none-feed-group"),
            pytest.param(
                "foobar", 1, [], "foobar", id="invalid-feed-group-int"),
            pytest.param("foobar", ["x", "y"],
                         None,
                         "foobar",
                         id="invalid-feed-group-list"),
            pytest.param(
                "foobar",
                "abc:xyz",
    ...
    def test_get_normalized_vulnerability_id(self, vuln_id, feed_group,
                                             nvd_refs, expected):
        assert (VulnerabilityMapper._try_get_normalized_vulnerability_id(
            vuln_id, feed_group, nvd_refs) == expected)
...
class TestEngineGovulnersDBMapper:
    ...
    def test_transform_cvss_score(self, cvss_dict, expected):
        assert EngineGovulnersDBMapper()._transform_cvss_score(
            cvss_dict) == expected
    ...
    def test_transform_cvss(self, vuln_id, cvss_dict, expected):
        assert EngineGovulnersDBMapper()._transform_cvss(vuln_id,
                                                         cvss_dict) == expected
