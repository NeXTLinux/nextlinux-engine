            ...
            [
                (
                    ...
                    CpeV2Vulnerability(
                        ...
                        product="package1",
                        version="1.0",
                        created_at=datetime.datetime(year=2021, month=1,
                                                     day=1),
                    ),
                ),
                (
...
def monkeypatched_records_for_namespace(monkeysession, request):
    ...
    def mock_matched_records_for_namespace(namespace_name: str,
                                           match_set: List[str]) -> List[str]:
        return request.param

    monkeysession.setattr(
        nextlinux_engine.services.policy_engine.engine.vulns.db.
        CpeDBQueryManager,
...
def monkeypatched_distro_mappings(monkeysession):
    ...
    def mock_distros_for(name: str, version: str,
                         flavor: str) -> List[DistroTuple]:
...
def mock_db_query_manager(monkeypatched_distro_mappings, request):
    ...
    class MockDbQueryManager:
        ...
        def matched_records_for_namespace(self, namespace_name: str,
...
def test_filter_secdb(
    ...
    filtered = filter_secdb_entries(image_distro=namespace,
                                    matches=image_matches,
                                    db_manager=mock_db_query_manager)
...
def test_cpes_for_images_packages(packages, mapped_cpes):
    ...
    def tuple_sort_key(
            cpe_tuple: Tuple[ImagePackage,
                             ImageCpe]) -> Tuple[str, str, str, str]:
        img = cpe_tuple[0]
        cpe = cpe_tuple[1]
        return (
            img.name,
    ...
    sorted_found = sorted(
        map(tuple_sort_key, cpes_for_image_packages(packages)))
