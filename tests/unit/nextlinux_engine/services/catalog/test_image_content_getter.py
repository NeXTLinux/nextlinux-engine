...
class TestMultipleContentTypesGetter:
    ...
    def initialize_storage_manager(self):
        manager.manager_singleton = {
            manager.DEFAULT_OBJECT_STORE_MANAGER_ID: "bar"
        }
    ...
    def test_is_content_type_match(self, request_types, content_type, expected,
                                   initialize_storage_manager):
        assert (MultipleContentTypesGetter(
            account_id="foo", content_types=request_types,
            image_digest="")._is_content_type_match(content_type) == expected)
