...
GOVULNERS_ARCHIVE_FILE_NAME = "grype_db_test_archive.tar.gz"
GOVULNERS_DB_VERSION = "3"

GOVULNERS_DB_DIR = "grype_db/"
...
def test_grype_wrapper_singleton(
        monkeypatch, production_grype_db_dir) -> GovulnersWrapperSingleton:
    """
    Creates a TestGovulnersWrapperSingleton, with attributes attributes for a mock production grype_db.
    ...
    """
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()

    grype_wrapper_singleton._grype_db_dir = production_grype_db_dir
    grype_wrapper_singleton._grype_db_version = GOVULNERS_DB_VERSION

    test_production_grype_db_engine = (
        grype_wrapper_singleton._init_latest_grype_db_engine(
            production_grype_db_dir, GOVULNERS_DB_VERSION))
...
def get_test_file_path(basename: str) -> str:
    ...
    return os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        TEST_DATA_RELATIVE_PATH, basename)
...
def mock_synced_dir(base_path, mock_checksum, include_engine_metadata) -> str:
    ...
    versioned_dir = os.path.join(grype_dir, GOVULNERS_DB_VERSION)
    ...
    shutil.copy(
        os.path.join(test_dir,
                     GovulnersWrapperSingleton.VULNERABILITY_FILE_NAME),
        versioned_dir,
    )
    shutil.copy(
        os.path.join(test_dir, GovulnersWrapperSingleton.METADATA_FILE_NAME),
        versioned_dir)
...
def mock_grype_db_session_maker():
    ...
    """
    db_connect = GovulnersWrapperSingleton.SQL_LITE_URL_TEMPLATE.format(
        "/does/not/exist")
...
def test_get_missing_grype_db_dir():
    ...
    with pytest.raises(ValueError,
                       match=GovulnersWrapperSingleton.
...
def test_move_grype_db_archive(tmp_path, grype_db_archive):
    ...
    grype_db_archive_copied_file_location = (
        grype_wrapper_singleton._move_grype_db_archive(grype_db_archive,
                                                       output_dir))
...
def test_move_missing_grype_db_archive(tmp_path):
    ...
    with pytest.raises(
        ...
        # Function under test
        grype_wrapper_singleton._move_grype_db_archive(missing_output_archive,
                                                       output_dir)
...
def test_open_grype_db_archive(grype_db_archive):
    ...
    expected_output_dir = os.path.join(parent_dir,
    ...
    latest_grype_db_dir = grype_wrapper_singleton._open_grype_db_archive(
        grype_db_archive, parent_dir, STAGED_VERSION_MOCK_CHECKSUM,
        GOVULNERS_DB_VERSION)
    ...
    assert os.path.exists(
        os.path.join(expected_output_dir, GOVULNERS_DB_VERSION))
    assert os.path.exists(
        os.path.join(
            expected_output_dir,
            GOVULNERS_DB_VERSION,
...
def test_write_engine_metadata_to_file(
        staging_grype_db_dir_no_engine_metadata):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()

    # Setup input var and create directories
    versioned_dir = os.path.join(staging_grype_db_dir_no_engine_metadata,
                                 GOVULNERS_DB_VERSION)
    if not os.path.exists(versioned_dir):
        os.makedirs(versioned_dir)
    ...
    expected_engine_metadata = {
        ...
        "grype_db_version": GOVULNERS_DB_VERSION,
...
def test_init_grype_db_engine(staging_grype_db_dir):
    ...
    latest_grype_db_engine = grype_wrapper_singleton._init_latest_grype_db_engine(
        staging_grype_db_dir, GOVULNERS_DB_VERSION)

    # Validate expected output
    assert str(latest_grype_db_engine.url
               ) == GovulnersWrapperSingleton.SQL_LITE_URL_TEMPLATE.format(
...
def test_init_latest_grype_db_engine(staging_grype_db_dir):
    ...
    # Validate output
    assert str(
        latest_grype_db_engine.url) == "sqlite:///{}".format(expected_output)
...
def test_init_grype_db(grype_db_parent_dir, grype_db_archive):
    ...
    # Setup expected output vars
    expected_output_dir = os.path.join(grype_db_parent_dir,
                                       STAGED_VERSION_MOCK_CHECKSUM)
    ...
    (
        ...
        grype_db_archive, STAGED_VERSION_MOCK_CHECKSUM, GOVULNERS_DB_VERSION)
...
def test_update_grype_db_staging(
    ...
    # Setup expected output vars
    expected_staging_output_dir = os.path.join(grype_db_parent_dir,
                                               STAGED_VERSION_MOCK_CHECKSUM)
    ...
    # Function under test
    grype_wrapper_singleton.update_grype_db(grype_db_archive,
                                            STAGED_VERSION_MOCK_CHECKSUM,
                                            GOVULNERS_DB_VERSION, True)
...
def test_unstage_grype_db(production_grype_db_dir_no_engine_metadata,
                          staging_grype_db_dir_no_engine_metadata):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()
    ...
    grype_wrapper_singleton._staging_grype_db_dir = (
        staging_grype_db_dir_no_engine_metadata)
    grype_wrapper_singleton._staging_grype_db_version = GOVULNERS_DB_VERSION
    ...
    assert (grype_wrapper_singleton._grype_db_dir ==
            production_grype_db_dir_no_engine_metadata)
    assert grype_wrapper_singleton._grype_db_version == GOVULNERS_DB_VERSION
...
def test_convert_grype_db_engine_metadata(
        production_grype_db_dir_no_engine_metadata):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()
...
def test_get_staging_grype_db_metadata(production_grype_db_dir,
                                       staging_grype_db_dir,
                                       metadata_file_name):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()
    ...
    # Setup expected output
    metadata_file_path = os.path.join(production_grype_db_dir,
                                      GOVULNERS_DB_VERSION, metadata_file_name)
...
def test_get_current_grype_db_metadata(production_grype_db_dir,
                                       staging_grype_db_dir,
                                       metadata_file_name):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()
    ...
    # Setup expected output
    metadata_file_path = os.path.join(staging_grype_db_dir,
                                      GOVULNERS_DB_VERSION, metadata_file_name)
    with open(metadata_file_path, "r") as read_file:
        expected_metadata = json.load(read_file)
...
def test_get_staging_proc_env(production_grype_db_dir, staging_grype_db_dir):
    ...
    assert result["GOVULNERS_DB_CACHE_DIR"] == staging_grype_db_dir


def test_get_production_proc_env(production_grype_db_dir,
                                 staging_grype_db_dir):
    # Create grype_wrapper_singleton instance
    grype_wrapper_singleton = TestGovulnersWrapperSingleton.get_instance()
    ...
    assert result["GOVULNERS_LOG_STRUCTURED"] == "1"
    assert result["GOVULNERS_DB_AUTO_UPDATE"] == "0"
    assert result["GOVULNERS_DB_CACHE_DIR"] == production_grype_db_dir
...
def test_query_vulnerabilities(
    ...
    assert (sorted(
        list(
            set(
                map(lambda result: result.GovulnersVulnerabilityMetadata.id,
...
class TestLocking:
    ...
    def test_simultaneous_read(self):
        ...
        """
        grype_wrapper = TestGovulnersWrapperSingleton.get_instance()
        ...
        a = Thread(
            ...
            args=(grype_wrapper, reader_1_name, instruction_queue,
                  output_queue),
        )
        b = Thread(
            target=self.hold_read_lock,
            args=(grype_wrapper, reader_2_name, instruction_queue,
                  output_queue),
        )
    ...
    def test_simultaneous_write_while_reading(self):
        ...
        reader = Thread(
            target=self.hold_read_lock,
            args=(grype_wrapper, reader_name, reader_instruction_queue,
                  output_queue),
        )
        writer = Thread(
            target=self.hold_write_lock,
            args=(grype_wrapper, writer_name, writer_instruction_queue,
                  output_queue),
        )
