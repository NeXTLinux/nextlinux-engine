...
from nextlinux_engine.clients import skopeo_wrapper
    ...
    def return_value(layer_media_type: str):
        manifest = {
            ...
            "config": {
                ...
                "digest":
                "sha256:b4382afec288f18a7255c91b9c5dfe37d66dc1aca7be8cefdbbd5b14ae794d3f",
                "size": 354,
            },
            "layers": [{
                "mediaType":
                layer_media_type,
                "digest":
                "sha256:0e31ee8f400716f940be9db76d6eedb1a903860385b4ca8f70513012d53fe1f1",
                "size":
                103066803,
                "urls": [
...
def test_fetch_oci_blobs_single_blob(monkeypatch, tmpdir, blobs):
    ...
    monkeypatch.setattr(skopeo_wrapper, "urlretrieve",
                        FakeUrlRetrieve(content=expected_blob_content))
...
def test_fetch_oci_blobs_it_bombs(monkeypatch, blobs):
    monkeypatch.setattr(skopeo_wrapper, "urlretrieve",
                        FakeUrlRetrieve(raises=TypeError))
...
def test_ensure_no_nondistributable_media_types(tmpdir, oci_index,
                                                oci_manifest):
    ...
    with open(oci_manifest_file_path, "r") as _f:
        assert _f.read() == oci_manifest(
            layer_media_type=expected_layer_media_type)
