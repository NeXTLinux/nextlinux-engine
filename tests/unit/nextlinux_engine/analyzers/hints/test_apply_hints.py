...
distro_hint_files = [
    "alpine_hints.json", "debian_hints.json", "rpm_hints.json"
]
...
class TestApplyHints:
    ...
    def log_to_stdout(self, monkeypatch):
        monkeypatch.setattr("nextlinux_engine.subsys.logger._log_to_stdout",
                            True)
    ...
    def patch_hints_read(self, monkeypatch):
        ...
        def _patch_read(hints):
            ...
            monkeypatch.setattr(
                "nextlinux_engine.analyzers.utils.get_hintsfile", _hints)
    ...
    def test_hints_defines_path(self, hints_filename, patch_hints_read,
                                get_hints):
        ...
        for pkg in hints["packages"]:
            if hints_filename in distro_hint_files:
                assert pkg["name"] in analyzer_report["package_list"][
                    pkg_type]["base"]
            else:
                assert (pkg["location"]
                        in analyzer_report["package_list"][pkg_type]["base"])

    def test_no_override_existing_entry(self, patch_hints_read, get_hints,
                                        caplog):
        ...
        assert (analyzer_report["package_list"]["pkgs.allinfo"]["base"]
                ["tzdata"]["version"] == "test-no-override")

    @pytest.mark.parametrize("hints_filename", hint_file_pkg_type_map.keys())
    def test_merge(self, hints_filename, get_hints, analyzer_report,
                   patch_hints_read):
        ...
        for pkg_type in report_pre_hints["package_list"].keys():
            ...
            if pkg_type == hints_pkg_type:
                assert len(post_hints) == len(pre_hints) + len(
                    hints["packages"])
