...
from nextlinux_engine.analyzers.gosbom.adapters import (
    _convert_gosbom_to_engine,
    _filter_artifacts,
)
...
def test_sbom(request):
    ...
    with open(
            os.path.join(module_path, test_name,
                         "{}.json".format(request.node.originalname))) as file:
...
class TestConvertGosbomToEngine:
    ...
    def test_filter_artifact_by_type(self, test_sbom,
                                     enable_package_filtering):
        findings = _convert_gosbom_to_engine(test_sbom,
                                             enable_package_filtering)
        for pkg_list in findings["package_list"]:
            if pkg_list == "pkgfiles.all":
                continue
            assert ("UNSUPPORTED-PACKAGE"
                    not in findings["package_list"][pkg_list]["base"].keys())
            assert ("SUPPORTED-PACKAGE"
                    in findings["package_list"][pkg_list]["base"].keys())
