...
def test_load_filepath_to_config(mock_default_config, tmpdir, config_key,
    ...
    assert (
        config["nextlinux_scanner_analyzer_config_file"] == output_dir_name +
        "/" + config_filename)
...
def test_load_policy_bundles(tmpdir, bundle_filenames, simulate_exception,
                             expected_bundles, expected_exceptions):
