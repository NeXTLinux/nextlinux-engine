...
class TestMavenVersionHandling(unittest.TestCase):
    ...
    def _check_ordering_(self, versions, increasing=True):
        ...
        for l, r in zip(range(len(mvs)), range(1, len(mvs))):
            print("lhs - actual: {}, canonical: {}".format(
                mvs[l].value, mvs[l]))
            print("rhs - actual: {}, canonical: {}".format(
                mvs[r].value, mvs[r]))
            ...
            self.assertLess(
                small,
                big,
                "Expected {} to be less than {}".format(
                    small.value, big.value),
            )
            self.assertEqual(
                small.compare_to(big),
                -1,
                "Expected {} to be less than {}".format(
                    small.value, big.value),
            )
            self.assertGreater(
                big,
                small,
                "Expected {} to be greater than {}".format(
                    big.value, small.value),
            )
            self.assertEqual(
                big.compare_to(small),
                1,
                "Expected {} to be greater than {}".format(
                    big.value, small.value),
            )
    ...
    def test_versions_qualifier_order(self):
        ...
        self._check_ordering_(reversed(self._versions_qualifier_),
                              increasing=False)

    def test_versions_number_order(self):
        self._check_ordering_(self._versions_number_, increasing=True)
        self._check_ordering_(reversed(self._versions_number_),
                              increasing=False)
    ...
    def _check_op_(self, v1, v2, op):
        ...
        if op == "==":
            self.assertEqual(
                mv1, mv2,
                "Expected {} and {} to be equal".format(mv1.value, mv2.value))
            self.assertEqual(
                mv1.compare_to(mv2),
                0,
                "Expected {} and {} to be equal".format(mv1.value, mv2.value),
            ...
            self.assertEqual(mv1.__hash__(), mv2.__hash__(),
                             "Expected hash code to be equal")
        elif op == ">":
            self.assertGreater(
                mv1,
                mv2,
            ...
            self.assertGreater(
                mv2, mv1,
                "Expected {} to be less than {}".format(mv1.value, mv2.value))
            self.assertLess(
                mv1, mv2,
                "Expected {} to be less than {}".format(mv1.value, mv2.value))
            self.assertEqual(
    ...
    def testVersionComparing(self):
        ...
        self._check_op_("1.0-SNAPSHOT", "1.0-beta-1",
                        ">")  # opposite of original test
