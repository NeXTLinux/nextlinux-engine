...
class TestApkVersionHandling(unittest.TestCase):
    ...
    def test_version_comparison(self):
        for lval, rval, tests in self.versions:
            for op, result in list(tests.items()):
                print(("{} {} {}, Expected: {}".format(lval, op, rval,
                                                       result)))
    ...
    def setUpClass(cls):
        if enable_training:
            print(
                "Executing result detection using apk from docker.io/alpine:latest"
            )
            ...
            try:
                for lval, rval, tests in cls.versions:
                    ...
                    out_op = out.strip()
                    print(("From apk test, got {} {} = {}".format(
                        lval, rval, out_op)))
                    for op, result in list(tests.items()):
                        if op == cls.ops[out_op]:
                            ...
                            tests[op] = False
                print(("Detected truth values for comparison are: {}".format(
                    cls.versions)))
            finally:
