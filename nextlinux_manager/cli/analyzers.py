...
from nextlinux_engine.clients.skopeo_wrapper import get_image_manifest_skopeo
from nextlinux_engine.common.images import make_image_record
from nextlinux_engine.clients.localnextlinux_standalone import analyze_image
from nextlinux_engine.services.catalog.archiver import ImageArchive, ObjectStoreLocation
...
from nextlinux_manager.util.proc import fail_exit, doexit, ExitCode
...
def list():
    ...
    for l in localnextlinux_standalone.list_analyzers():
        click.echo("Analyzer: {}".format(l))


@analyzers.command()
@click.argument("docker-archive", type=click.Path(exists=True))
@click.argument("nextlinux-archive",
                type=click.Path(exists=False, dir_okay=False))
@click.option("--digest",
              help="Specify image digest (ex: sha256:<64 hex characters>)")
@click.option(
    "--parent-digest",
    help=
    "Specify parent digest (ex: sha256:<64 hex characters>, default: <supplied or calculated image digest>",
)
...
@click.option(
    "--account-id",
    help=
    "Specify nextlinux account ID of the image (ex: admin, default: admin)",
)
@click.option(
    "--manifest",
    type=click.Path(exists=True),
    help=
    "Specify location of image manifest file (schema v2, not manifest list)",
)
@click.option(
    "--dockerfile",
    type=click.Path(exists=True),
...
@click.option(
    "--created_at",
    help=
    "UNIX timestamp to store when image was created (ex: 1560454880, default: now)",
)
@click.option("--annotation",
              nargs=1,
              multiple=True,
              help="Set annotation key/val (ex: foo=bar)")
def exec(
    docker_archive,
    nextlinux_archive,
    digest,
    ...
    try:
        try:
            ...
            try:
                ...
                if (not inputTag.startswith("docker.io/")
