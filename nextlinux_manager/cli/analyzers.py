...
from nextlinux_engine.clients.skopeo_wrapper import manifest_to_digest
...
from nextlinux_manager.util.proc import fail_exit
...
@click.argument("nextlinux-archive", type=click.Path(exists=False, dir_okay=False))
...
@click.option(
    "--account-id",
    help="Specify nextlinux account ID of the image (ex: admin, default: admin)",
)
@click.option(
    "--manifest",
    ...
    try:
        try:
            ...
            if os.path.exists(nextlinux_archive):
                raise Exception(
                    "the supplied nextlinux archive file ({}) already exists, please remove and try again".format(
                        nextlinux_archive
                    )
                )
        ...
        image_content_data = {}
        for content_type in (
            nextlinux_engine.common.image_content_types
