...
from nextlinux_manager.util.proc import ExitCode, doexit
...
@click.option(
    "--nextlinux-module", help="Module to list services for", default="nextlinux_engine"
)
...
@click.option(
    "--nextlinux-module",
    nargs=1,
    help="Name of nextlinux module to call DB routines from (default=nextlinux_engine)",
)
@click.option(
...
def start(
    ...
    try:
        ...
        # preflight - db checks
        try:
            db_params = nextlinux_engine.db.entities.common.get_params(localconfig)
                ...
                if code_versions and db_versions:
                    if code_versions["db_version"] != db_versions["db_version"]:
                        if not no_auto_upgrade and "nextlinux-catalog" in services:
                            ...
                            logger.warn(
                                "this version of nextlinux-engine requires the nextlinux DB version ({}) but we discovered nextlinux DB version ({}) in the running DB - it is safe to run the upgrade while seeing this message - will retry for {} more seconds.".format(
