...
class NextlinuxLogWatcher(RegexMatchingEventHandler):
    ...
    def do_close(self, event):
        if event.src_path in self.files and self.files[
                event.src_path]["filehandle"]:
    ...
    def on_modified(self, event):
        ...
        if self.files[event.src_path]["filehandle"]:
            ...
            self.files[event.src_path]["filetell"] = self.files[
                event.src_path]["filehandle"].tell()
...
@click.option(
    ...
    nargs=1,
    help=
    "Comma-separated list of configuration file sections to skip specific validation processing (e.g. services,credentials,webhooks)",
)
@click.option(
...
def start(
    ...
    if services:
        ...
        input_services = os.getenv("NEXTLINUX_ENGINE_SERVICES",
                                   "").strip().split()
    ...
    try:
        ...
        try:
            ...
            nextlinux_manager.util.db.connect_database(db_params,
                                                       db_retries=300)
