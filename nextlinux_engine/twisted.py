...
from nextlinux_engine.apis.ssl import _load_ssl_cert, _load_ssl_key
...
class WsgiApiServiceMaker(object):
    ...
    def _build_api_service(self):
        ...
        self._add_resource(
            self.nextlinux_service.__service_api_version__.encode("utf-8"), wsgi_site
        )
        ...
        if (
            str(self.nextlinux_service.configuration.get("ssl_enable", "")).lower()
            == "true"
        ):
            try:
                ssl_data = {
                    "ssl_cert": _load_ssl_cert(
                        self.nextlinux_service.configuration["ssl_cert"]
                    )
                    if "ssl_cert" in self.nextlinux_service.configuration
                    else None,
                    "ssl_chain": _load_ssl_cert(
                        self.nextlinux_service.configuration["ssl_chain"]
                    )
                    if "ssl_chain" in self.nextlinux_service.configuration
                    else None,
                    "ssl_key": _load_ssl_key(
                        self.nextlinux_service.configuration["ssl_key"]
                    )
                    if "ssl_key" in self.nextlinux_service.configuration
                    else None,
                }
