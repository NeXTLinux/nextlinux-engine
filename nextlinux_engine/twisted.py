...
enable_thread_dumper = (os.getenv("NEXTLINUX_ENABLE_DANGEROUS_THREAD_DUMP_API",
                                  "false").lower() == "true")
...
def _load_config(config_option, validate_params=None):
    try:
        ...
        localconfig.load_config(configdir=configdir,
                                configfile=configfile,
                                validate_params=validate_params)
        my_config = localconfig.get_config()
        my_config["myservices"] = []
        logger.spew("localconfig=" +
                    json.dumps(my_config, indent=4, sort_keys=True))
        return my_config
...
class WsgiApiServiceMaker(object):
    ...
    def _check_enabled(self):
        if (not self.global_configuration.get("services", {}).get(
                self.service_cls.__service_name__, {}).get("enabled", False)):
            log.err(
                "Service {} not enabled in configuration file: shutting down".
                format(self.service_cls.__service_name__))
            sys.exit(0)

    def _init_config(self, options):
        _validate_options(options)
        self.global_configuration = _load_config(options["config"])
        self.service_config = self.global_configuration.get(
            "services", {}).get(self.service_cls.__service_name__, {})
    ...
    def _default_version_rewrite(self, request):
        try:
            if request.postpath:
                ...
                if (request.postpath[0] not in self.resource_nodes.keys()
