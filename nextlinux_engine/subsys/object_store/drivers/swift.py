...
class SwiftObjectStorageDriver(ObjectStorageDriver):
        ...
        try:
            ...
            if resp["success"]:
                ...
                raise DriverConfigurationError(
                    Exception(
                        "Got unsuccessful response from stat operation against service: {}".format(
                            resp
                        )
                    )
                )
    ...
    def _build_key(self, userId, usrBucket, key):
        return self._key_format.format(
            prefix=self.prefix, userid=userId, container=usrBucket, key=key
        )
    ...
    def exists(self, uri):
        try:
            ...
            resp = self.client.download(
                container=container,
                objects=[key],
                options={"out_file": "-", "no_download": True},
            )
            for obj in resp:
                if (
                    "success" in obj
                    and obj["success"]
                    and obj["action"] == "download_object"
                ):
                    return True
                elif obj["action"] == "download_object" and not obj["success"]:
                    return False
