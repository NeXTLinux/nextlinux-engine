...
from nextlinux_engine.configuration.localconfig import InvalidOauthConfigurationError
from nextlinux_engine.db import db_services
from nextlinux_engine.db import initialize as initialize_db
...
class UserFacingApiService(ApiService):
    ...
    def _process_api_spec(self):
        try:
            ...
            if missing:
                raise Exception(
                    "API Spec validation error: All operations must have a x-nextlinux-authz-action label. Missing for: {}".format(
                        missing
                    )
                )
            else:
