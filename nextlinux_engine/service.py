...
class BaseService(object, metaclass=ServiceMeta):
    ...
    def _process_stage_handlers(self, stage):
        logger.info("Processing init handlers for bootsrap stage: {}".format(
            stage.name))
        handlers = self.lifecycle_handlers.get(stage, [])
        logger.debug("Executing {} stage {} handlers".format(
            len(handlers), stage.name))
        for handler_fn, handler_args in handlers:
            try:
                logger.debug("Invoking handler: {} with args {}".format(
                    handler_fn.__name__, handler_args))
                if handler_args is not None:
                    handler_fn(*handler_args)
                else:
                    handler_fn()
                logger.debug("Handler: {} completed successfully".format(
                    handler_fn.__name__, handler_args))
            except Exception as ex:
                logger.exception(
                    "Pre-Stage Handler {} for service pre_config raised exception"
    ...
    def _configure(self):
        ...
        env_setting = (not os.environ.get("NEXTLINUX_ENGINE_DISABLE_MONITORS",
                                          "false").lower() == "true")
    ...
    def _db_connect(self):
        ...
        if not self.db_connect:
            logger.info(
                "DB Connection disabled in configuration for service {}. Skipping db init"
                .format(self.__service_name__))
            return True

        logger.info("Initializing database")
        # connect to DB
        try:
            initialize_db(localconfig=self.global_configuration,
        ...
        if self.require_system_user:
            ...
            for count in range(1, max_retries):
                try:
                    ...
                    self.global_configuration["system_user_auth"] = (None,
                                                                     None)
                    gotauth = False
    ...
    def get_monitor_thread(self, monitor_thread_wrapper=None):
        ...
        if self.task_handlers_enabled:
            if monitor_thread_wrapper:
                t = monitor_thread_wrapper(self.monitor_fn,
                                           **self.monitor_kwargs)
            else:
                t = threading.Thread(target=self.monitor_fn,
                                     kwargs=self.monitor_kwargs)
...
class ApiService(BaseService):
    ...
    def _register_instance_handlers(self):
        ...
        self.register_handler(LifeCycleStages.pre_bootstrap,
                              self.initialize_api, None)
    ...
    def _init_wsgi_app(self,
        ...
        try:
            ...
            flask_app.register_error_handler(NextlinuxApiError,
    ...
    def initialize_api(self):
        ...
        logger.info("Initializing API from: {}/{}".format(
            self.__spec_dir__, self.__spec_file__))
        if (self.configuration["listen"] and self.configuration["port"]
                and self.configuration["endpoint_hostname"]):
            if not self._api_application:
                self._api_application = self._init_wsgi_app(
                    self.__service_name__, self.__spec_dir__,
                    self.__spec_file__)
...
class UserFacingApiService(ApiService):
    ...
    def _process_api_spec(self):
        try:
            ...
            missing = [
                x for x in filter(lambda x: x[1] is None, actions.items())
            ]
            if missing:
                raise Exception(
