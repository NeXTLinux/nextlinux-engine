...
class BasicApiClient(object):
    ...
    def call_api(
        ...
        if path_params:
            path_params = {
                name: urlparse.quote(value) for name, value in path_params.items()
            }
        ...
        logger.debug(
            "Dispatching: url={url}, headers={headers}, body={body}, params={params}".format(
                url=final_url,
        ...
        try:
            ...
            logger.error(
                "Failed client call to url: {}. Response: {}".format(
...
class AuthorizationRequest(SimpleJsonModel):
    ...
    def from_json(cls, json_dict):
        ...
        if type(json_dict.get("actions")) != list:
            raise ValueError(
                "Incorrect actions type, must be a list, found: {}".format(
                    type(json_dict.get("actions"))
                )
            )

        ar = AuthorizationRequest(
            Principal.from_json(json_dict.get("principal")),
            [Action.from_json(act_obj) for act_obj in json_dict.get("actions")],
        )
        return ar
...
class AuthorizationDecision(SimpleJsonModel):
    ...
    def from_json(cls, json_dict):
        ...
        if type(json_dict.get("allowed")) != list:
            raise ValueError(
                "Incorrect allowed type, must be a list, found: {}".format(
                    type(json_dict.get("actions"))
                )
            )
        ...
        if type(json_dict.get("denied")) != list:
            raise ValueError(
                "Incorrect denied type, must be a list, found: {}".format(
                    type(json_dict.get("actions"))
                )
            )
...
class Action(SimpleJsonModel):
    ...
    def from_json(cls, json_dict):
        ...
        act = Action(
            json_dict.get("domain"), json_dict.get("action"), json_dict.get("target")
        )
        return act
    ...
    def __hash__(self):
        return (
            self.domain.__hash__(),
            self.action.__hash__(),
            self.target.__hash__(),
        ).__hash__()
...
class AuthzPluginHttpClient(BasicApiClient):
    ...
    def delete_domain(self, domain: str):
        resp = self.call_api(
            requests.delete, "domains", query_params={"domain": domain}
        )
    ...
    def initialize_principal(self, principal: str):
        ...
        resp = self.call_api(
            requests.post, "principals", body=json.dumps(req.to_json())
        )
        if resp.status_code in [200, 204]:
            return True
        else:
    ...
    def delete_principal(self, principal: str):
        resp = self.call_api(
            requests.delete, "principals", query_params={"principal": principal}
        )
