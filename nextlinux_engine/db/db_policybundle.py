...
def set_active_policy(policyId, userId, session=None):
    ...
    result = (
        session.query(PolicyBundle)
        .filter(PolicyBundle.userId == userId)
        .filter(PolicyBundle.policyId == policyId)
        .first()
    )
    if result:
...
def update_record(input_record, session=None):
    ...
    our_result = (
        session.query(PolicyBundle)
        .filter_by(policyId=input_record["policyId"], userId=input_record["userId"])
        .first()
    )
    if our_result:
