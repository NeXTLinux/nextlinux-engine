...
def get_enabled_services(servicename, skipcache=False):
    ...
    if not ret:
        logger.debug(
            "no services of type ("
            + str(servicename)
            + ") are yet available in the system"
        )
...
def choose_service(servicename, skipcache=False):
    ...
    if not ret:
        logger.debug(
            "no service of type ("
            + str(servicename)
            + ") is yet available in the system"
        )
...
def get_service_endpoint(servicename, api_post=None):
    ...
    try:
        ...
        if not service_record:
            raise Exception(
                "cannot locate registered and available service in config/DB: "
                + servicename
            )
        else:
            ...
            if endpoint:
                ...
                raise Exception(
                    "cannot load valid endpoint from service record: " + servicename
                )

    except Exception as err:
        raise Exception(
            "could not find valid endpoint for service ("
            + servicename
            + ") - exception: "
            + str(err)
        )
