...
def handle_tar_error_post(unpackdir=None,
                          rootfsdir=None,
    ...
    logger.debug(
        "handling post with metadata: {}".format(handled_post_metadata))
    if handled_post_metadata.get("temporary_file_adds", []):
        for tfile in handled_post_metadata.get("temporary_file_adds", []):
            ...
            if os.path.exists(rmfile):
                logger.debug(
                    "removing temporary image file: {}".format(rmfile))
                if os.path.isfile(rmfile):
                    os.remove(rmfile)

    if handled_post_metadata.get("temporary_dir_adds", []):
        for tfile in sorted(handled_post_metadata.get("temporary_dir_adds",
                                                      []),
                            reverse=True):
            rmfile = os.path.join(rootfsdir, tfile)
            if os.path.exists(rmfile):
                logger.debug(
                    "removing temporary image dir, only if terminal (empty): {}"
                    .format(rmfile))
...
def tree_create_branch(ftree,
                       id,
                       data,
                       stree=None,
                       populate_intermediate_nodes=False):
    ftoks = id.split("/")
    ...
    try:
        ...
        for l in layers:
            ...
            for member in tarfiles[l].getmembers():
                ...
                if whopqpatt.match(os.path.basename(filename)):
                    ...
                    logger.debug("WH handler {} {}".format(
                        filename, time.time()))
        ...
        with tarfile.open(os.path.join(unpackdir, "squashed.tar"),
                    ...
                    if (ftree[filename].data["exists"]
                        ...
                        if member.isreg():
                            ...
                            if ftree[filename].data["hl_replace"]:
                                deferred_hardlinks[filename] = ftree[
                                    filename].data
                            else:
                                oltf.addfile(member)
                        else:
            ...
            for filename in deferred_hardlinks.keys():
                ...
                member = tarfiles_members[l].get(filename)
                logger.debug("deferred hardlink {}".format(
                    ftree[filename].data))
                try:
                    logger.debug(
...
def make_staging_dirs(rootdir, use_cache_dir=None):
    if not os.path.exists(rootdir):
        raise Exception("passed in root directory must exist (" +
                        str(rootdir) + ")")
    ...
    for k in list(ret.keys()):
        ...
        try:
            ...
            raise Exception("unable to prep staging directory - exception: " +
                            str(err))
...
def _rmtree_error_handler(infunc, inpath, inerr):
    ...
    try:
        ...
        logger.warn(
            "unable to change permissions in error handler for path {} in shutil.rmtree"
            .format(inpath))
    finally:
        try:
            infunc(inpath)
            ...
            logger.debug(
                "unable to remove in error handler for path {} - this will be retried"
                .format(err))
        ...
        if not myconfig.get("keep_image_analysis_tmpfiles", False):
            try:
                if os.path.exists(staging_dirs[k]):
                    logger.debug("removing dir: " + k + " : " +
                                 str(staging_dirs[k]))
                    rmtree_force(staging_dirs[k])
            except Exception as err:
                raise Exception(
                    "unable to delete staging directory - exception: " +
                    str(err))
        else:
            logger.debug(
                "keep_image_analysis_tmpfiles is enabled - leaving analysis tmpdir in place {}"
                .format(staging_dirs))

    return True


def pull_image(staging_dirs,
               pullstring,
               registry_creds=None,
               manifest=None,
...
def get_image_config(copydir, cachedir, image_id) -> dict:
    ...
    if not nfile:
        raise Exception(
            "could not find final digest - no blob config file found in digest file: {}"
            .format(dfile))
...
def analyze_image(
    ...
    try:
        ...
        logger.debug("timing: total unpack time: {} - {}".format(
            pullstring,
            time.time() - timer))
...
class AnalysisError(AnchoreException):
    ...
    def __repr__(self):
        return "{} ({}) - exception: {}".format(self.msg, self.pull_string,
                                                self.cause)
    ...
    def to_dict(self):
        return {
            ...
            dict((
                key,
                "{}...(truncated)".format(value[:256]) if key == "cause"
                and isinstance(value, str) and len(value) > 256 else value,
