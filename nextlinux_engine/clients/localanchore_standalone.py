...
from nextlinux_engine.analyzers import manager as analyzer_manager
from nextlinux_engine.analyzers.manager import AnalysisResult
...
try:
    ...
    from nextlinux_engine.subsys import logger  # pylint: disable=C0412
except:
    import logging
...
def analyze_image(
    ...
    for image_detail in image_details:
        try:
            ...
            timer = time.time()
            analysis_result = run_nextlinux_analyzers(
                staging_dirs,
                imageDigest,
                imageId,
                localconfig,
...
def get_nextlinuxlock(lockId=None, driver=None):
    ...
    if not driver or driver in ["localnextlinux"]:
        ...
        nextlinux_data_dir = "{}/.nextlinux".format(os.getenv("HOME", "/tmp/nextlinuxtmp"))
    ...
    try:
        for src, dst in [
            ...
            (
                os.path.join(localconfig["service_dir"], "nextlinux_config.yaml"),
