...
def verify_import_manifest_content(db_session, operation_id: str,
    ...
    if import_manifest.contents.packages:
        found = (db_session.query(ImageImportContent).filter(
    ...
    if import_manifest.contents.parent_manifest:
        found = (db_session.query(ImageImportContent).filter(
            ImageImportContent.operation_id == operation_id,
            ImageImportContent.digest ==
            import_manifest.contents.parent_manifest,
            ImageImportContent.content_type ==
            ImportTypes.parent_manifest.value,
        ).one_or_none())
        if found is None:
    ...
    for t in REQUIRED_IMPORT_TYPES:
        if getattr(import_manifest.contents, t.value) is None:
            raise api_exceptions.BadRequest(
                "import manifest must have digest for content type {} present",
                t.value)
...
def internal_manifest_from_external(
        ...
        content_records: list) -> InternalImportManifest:
    """
...
def add_import_annotations(import_manifest: ImportManifest,
                           annotations: dict = None):
    """
    ...
    annotations[
        IMPORT_OPERATION_ANNOTATION_KEY] = import_manifest.operation_uuid
    return annotations
