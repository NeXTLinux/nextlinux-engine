...
from nextlinux_engine.services.catalog import utils
from nextlinux_engine.subsys import logger, notifications, object_store, taskstate
from nextlinux_engine.subsys.events.util import analysis_complete_notification_factory
...
def repo(dbsession, request_inputs, bodycontent={}):
    ...
    try:
        if method == "POST":
            ...
            try:
                rc = nextlinux_engine.services.catalog.service.schedule_watcher(
                    "repo_watcher"
                )
...
def resolve_final_image_info(
    ...
    try:
        ...
        fail_event = nextlinux_engine.subsys.events.ImageRegistryLookupFailed(
            user_id=account_id,
            image_pull_string=input_string,
            data=err.__dict__,
        )
...
def validate_image_size(image_info):
    if not is_image_valid_size(image_info):
        ...
        raise BadRequest(
            ...
            detail={
                "requested_image_compressed_size_mb": nextlinux_utils.bytes_to_mb(
                    ...
                    if not image_record:
                        ...
                        try:
                            ...
                            if (
                                ...
                                == nextlinux_engine.subsys.taskstate.complete_state(
                                    "analyze"
                                )
                                and generate_event
                            ):
...
def is_image_valid_size(image_info):
    ...
    if (
        ...
        and nextlinux_utils.bytes_to_mb(compressed_image_size, round_to=2)
        > max_compressed_image_size_mb
    ):
        return False
    else:
