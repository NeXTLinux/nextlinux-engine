...
def add_event(event):
    ...
    try:
        ...
        logger.exception(
            "Uncaught exception from event emitter: {}".format(event))
...
class Artifact(JsonSerializable):
    ...
    class ArtifactV1Schema(Schema):
        ...
        source = fields.Nested(ObjectStoreLocation.ObjectStoreLocationV1Schema,
                               allow_none=True)
        dest = fields.Nested(TarballLocation.TarballLocationV1Schema,
                             allow_none=True)
...
class ImageArchive(object):
    ...
    def __enter__(self):
        if not self._tarfile or self._tarfile.closed:
            self._tarfile = tarfile.open(name=self.backing_file_path,
                                         mode="{}:gz".format(self._tar_mode))
...
class ImageAnalysisArchiver(object):
    ...
    def _get_globals(self, session):
        ...
        rules = (session.query(ArchiveTransitionRule).filter_by(
            transition=ArchiveTransitions.archive, system_global=True).all())
        return rules

    def _get_account_rules(self, session):
        # Select rules that are archive transitions, and either global, or specific to this account
        rules = (session.query(ArchiveTransitionRule).filter_by(
            transition=ArchiveTransitions.archive, account=self.account).all())
        return rules
    ...
    def _process_rules(self, rules, session):
        ...
        def lookup_tags(account, digest):
            return (session.query(CatalogImageDocker).filter_by(
                userId=account, imageDigest=digest).all())
    ...
    def _do_archive(self, to_archive):
        for img_digest in to_archive:
            logger.info("Archiving image {}/{}".format(self.account,
                                                       img_digest))
            try:
                if not DRY_RUN_MODE:
                    t = ArchiveImageTask(account=self.account,
                                         image_digest=img_digest)
                    status, msg = t.run()
                    logger.info(
                        "Archive task result: status={}, detail={}".format(
                            status, msg))

                    if status == "archived":
                        logger.info(
                            "Deleting source analysis for image {} after archiving"
                            .format(img_digest))
                        if self.delete_source:
                            ...
                            with session_scope() as session:
                                ...
                                if http_code not in [200, 204]:
                                    logger.error(
                                        "Could not delete image analysis: {}".
                                        format(resp))
                                else:
                                    logger.info(
                                        "Deleted image analysis for {} successfully"
                                        .format(img_digest))
                else:
                    logger.info(
                        "Archive task DRY_RUN mode enabled via {}, would have archived image {}/{}"
                        .format(DRY_RUN_ENV_VAR, self.account, img_digest))
            except Exception as ex:
    ...
    def run(self):
        try:
            ...
            logger.exception(
                "Unexpected exception caught in global archive transition rule handling for account {}"
                .format(self.account))
        ...
        try:
            ...
            logger.exception(
                "Unexpected exception caught in account-local archive transition rule handling for account {}"
                .format(self.account))
    ...
    def _find_candidates(self, session: Session, rule: ArchiveTransitionRule):
        ...
        if (rule.system_global and rule.max_images_per_account is not None
            ...
            if old_images_exceeding_max_count:
                ...
                logger.info(
                    "Transitioning {} images according to max_images_per_account setting"
                    .format(images_exceeding_max_count))
        return candidates
    ...
    def _evaluate_tag_history_and_exclude(self, rule, image_tuple_generator):
        ...
        for catalog_image_docker, catalog_image in image_tuple_generator:
            logger.debug("Checking tag: {}, image: {}".format(
            ...
            if self._is_exclude_match(rule, catalog_image,
                                      catalog_image_docker):
                logger.debug(
                    "Image is excluded from archive transition rule: {}".
                    format(catalog_image_docker))
            ...
            if (catalog_image_docker.registry != current_tag.registry
    ...
    def _is_exclude_match(self, rule, catalog_image, catalog_image_docker):
        ...
        return (fnmatch.filter([catalog_image_docker.registry],
                               rule.exclude_selector_registry)
                and fnmatch.filter([catalog_image_docker.repo],
                                   rule.exclude_selector_repository)
                and fnmatch.filter([catalog_image_docker.tag],
                                   rule.exclude_selector_tag))
...
class ArchivedAnalysisDeleter(ImageAnalysisArchiver):
    ...
    def run(self):
        with session_scope() as session:
            logger.debug(
                "Evaluating archive deletion rules for account: {}".format(
                    self.account))
            rules = (session.query(ArchiveTransitionRule).filter_by(
                account=self.account,
                transition=ArchiveTransitions.delete).all())

            def lookup_archived_tags(account, digest):
                img = (session.query(ArchivedImage).filter_by(
                    account=account, imageDigest=digest).one_or_none())
                return img.tags()
        ...
        for img_digest in to_delete:
            ...
            try:
                if not DRY_RUN_MODE:
                    ...
                    status, msg = t.run()
                    logger.info(
                        "Archive deletion task result: status={}, detail={}".
                        format(status, msg))
                else:
                    logger.info(
                        "Archive deletion task DRY_RUN mode enabled via {}, would have deleted archived image {}/{}"
                        .format(DRY_RUN_ENV_VAR, self.account, img_digest))
            except Exception as ex:
                logger.exception(
                    "Caught unhandled exception in archive deletion task")
...
class ArchiveTransitionTask(object):
    ...
    class TagRuleMatchMerger(object):
        ...
        * All tags for an image digest must match at least one rule for the image to be in the match set.
        ...
        def __init__(self, task_id, account: str,
                     image_tag_lookup_callback_fn: callable):
            """
            ...
            for tag, img in matched_tags:
                if tag.imageDigest not in self.image_tags_full_match:
                    if tag.imageDigest not in self.image_tags:
                        ...
                        full_tag_set = self.tag_lookup_fn(
                            self.account, tag.imageDigest)
                    ...
                    if tag.imageDigest in self.image_tags_subset_matched:
                        self.image_tags_subset_matched[tag.imageDigest].append(
                            tag)
                    else:
                        self.image_tags_subset_matched[tag.imageDigest] = [tag]

                    if set(self.image_tags_subset_matched[
                            tag.imageDigest]) == set(
                                self.image_tags[tag.imageDigest]):
                        # Move the image to the other collection
                        self.image_tags_full_match[
                            tag.
                            imageDigest] = self.image_tags_subset_matched.pop(
                                tag.imageDigest)
    ...
    def run(self):
        try:
            archiver = ImageAnalysisArchiver(self.account,
                                             self.parent_task_id,
                                             delete_source=True)
            archiver.run()
        except Exception as ex:
            logger.exception(
                "Unhandled exception caught from analysis archiver")

        try:
            deleter = ArchivedAnalysisDeleter(self.account,
                                              self.parent_task_id)
            deleter.run()
        except Exception as ex:
            logger.exception(
                "Unhandled exception caught from analysis archive deleter")
...
class DeleteArchivedImageTask(object):
    ...
    def run(self):
        logger.debug("Starting archive deletion process for image {}".format(
            self.image_digest))
        ...
        try:
            ...
            add_event(
                ImageArchiveDeleted(self.account, self.image_digest, self.id))
            return result
        except Exception as err:
            logger.exception("Failed archive deletion execution")
            add_event(
                ImageArchiveDeleteFailed(self.account,
                                         self.image_digest,
                                         task_id=self.id,
                                         err=str(err)))
            raise
    ...
    def _execute(self):
        ...
        """
        with session_scope() as session:
            rec = db_archived_images.get(session, self.account,
                                         self.image_digest)
            if not rec:
                raise MetadataNotFound("/".join(
        ...
        try:
            logger.debug("Deleting archive object: {}/{}".format(
        ...
        with session_scope() as session:
            logger.debug("Deleting archive records for {}/{}".format(
                self.account, self.image_digest))
            db_archived_images.delete(session, self.account,
                                      [self.image_digest])
...
class RestoreArchivedImageTask(object):
    ...
    def start(self):
        ...
        try:
            ...
            logger.exception(
                "Error executing restore of image analysis from archive")
        ...
        if m:
            ...
            try:
                ...
                with ImageArchive.for_reading(tf.name) as img_archive:
                    ...
                    logger.debug("Using manifest: {}".format(
                        img_archive.manifest))
    ...
    def restore_artifacts(self, img_archive: ImageArchive,
                          dest_mgr: ObjectStorageManager) -> bool:
        """
        ...
        for artifact in img_archive.manifest.artifacts:
            if artifact.metadata and "record" in artifact.metadata:
                ...
                if r_type == "policy_evaluation":
                    try:
                        ...
                        logger.warn(
                            "Could not insert records of type: {} due to exception: {}"
                            .format(r_type, ex))
                        continue
            ...
            if artifact.source:
                ...
                if not data:
                    logger.error("Could not get data for {}".format(
                        artifact.name))
                    raise Exception("Archive data unavailable for {}".format(
                        artifact.name))
                ...
                logger.debug(
                    "Skipping load of archived data: {} due to no source information in manifest"
    ...
    def _reload_policy_engine(self, manifest: ArchiveManifest) -> bool:
        ...
        try:
            pe_client = internal_client_for(PolicyEngineClient,
                                            userId=self.account)
            fetch_url = ("catalog://" + self.account + "/analysis_data/" +
                         self.image_digest)
...
class ArchiveImageTask(object):
    ...
    def __init__(self, account=None, image_digest=None, parent_task_id=None):
        ...
        self.required_artifacts = [
            Artifact(
                name="analysis",
                source=ObjectStoreLocation(bucket="analysis_data",
                                           key=self.image_digest),
                dest=None,
                metadata={},
            ),
            Artifact(
                name="image_content",
                source=ObjectStoreLocation(bucket="image_content_data",
                                           key=self.image_digest),
                dest=None,
                metadata={},
            ),
            Artifact(
                name="image_manifest",
                source=ObjectStoreLocation(bucket="manifest_data",
                                           key=self.image_digest),
                dest=None,
                metadata={},
            ),
    ...
    def run(self, merge=False):
        ...
        """
        logger.debug("Starting archiving process for image {}".format(
            self.image_digest))
        ...
        try:
            with session_scope() as session:
                found = db_archived_images.get(session, self.account,
                                               self.image_digest)
                ...
                if not catalog_img_dict:
                    raise Exception(
                        "Could not locate an image with digest {} in account {}"
                        .format(self.image_digest, self.account))
                else:
                    self._catalog_record = catalog_img_dict

                if (catalog_img_dict.get("image_status") != "active" or
                        catalog_img_dict.get("analysis_status") != "analyzed"):
                    raise Exception(
                        'Invalid image record state. Image must have "analysis_status"="analyzed" and "image_status"="active". Found {} and {}'
                        .format(
                ...
                # Add the new record
                img = ArchivedImage.from_catalog_image(catalog_img_dict,
                                                       cascade=True)
                if merge and found:
            ...
            add_event(
                ImageArchivingFailed(self.account,
                                     self.image_digest,
                                     self.id,
                                     err=str(ex)))
            return "error", str(ex)
    ...
    def _execute(self):
        ...
        with session_scope() as session:
            record = db_archived_images.get(session, self.account,
                                            self.image_digest)

            if not record:
                raise Exception(
                    "No analysis archive record found to track state")

            try:
                with tempfile.TemporaryDirectory(dir=localconfig.get_config().
                                                 get("tmp_dir")) as tempdir:
                    with ImageArchive.for_writing(
                            os.path.join(
                                tempdir,
                                "analysis_archive.tar.gz")) as img_archive:
                        img_archive.account = self.account
                        img_archive.image_digest = self.image_digest

                        if self._catalog_record.get("image_detail"):
                            image_id = self._catalog_record.get(
                                "image_detail")[0]["imageId"]
                        else:
                            image_id = None

                        img_archive.manifest.metadata = {
                            "versions":
                            localconfig.get_versions(),
                            "image_id":
                            image_id,
                            "image_record":
                            json.dumps(self._catalog_record, sort_keys=True),
                        }

                        self.archive_required(src_obj_mgr,
                                              self.required_artifacts,
                                              img_archive)

                        try:
                            vuln_artifacts = self.archive_vuln_history(
                                img_archive)
                        except:
                    ...
                    add_event(
                        ImageArchived(self.account, self.image_digest,
                                      self.id))
                ...
                if data_written:
                    logger.info(
                        "Cleaning up after failed analysis archive task for {}/{}"
                        .format(self.account, self.image_digest))
                    try:
                        resp = dest_archive_mgr.delete(self.account,
                                                       record.manifest_bucket,
                                                       record.manifest_key)
                    except Exception as ex:
                        logger.warn(
                            "Could not delete the analysis archive tarball in storage. May have leaked. Err: {}"
                            .format(ex))
    ...
    def archive_required(self, src_mgr: ObjectStorageManager, artifacts: list,
                         img_archive: ImageArchive) -> list:
        """

        :return: tuple of (manifest (dict), bucket (str), key (str))
        """
        for artifact in artifacts:
            data = src_mgr.get(self.account, artifact.source.bucket,
                               artifact.source.key)
    ...
    def archive_policy_evaluations(self, src_obj_mgr: ObjectStorageManager,
                                   img_archive: ImageArchive, session) -> list:
        """
        ...
        for eval_rec in policy_evaluations:
            ...
            artifact.source = ObjectStoreLocation(bucket="policy_evaluations",
                                                  key=eval_rec["evalId"])

            eval_content = src_obj_mgr.get(self.account,
                                           artifact.source.bucket,
                                           artifact.source.key)
            ...
            meta = {
                ...
                "completed_at":
                datetime_to_rfc3339(datetime.datetime.utcnow()),
                "bytes_copied": len(eval_content),
            }

            artifacts.append(artifact)
            img_archive.add_artifact(artifact.name,
                                     source=None,
                                     data=eval_content,
                                     metadata=meta)
    ...
    def archive_vuln_history(self, img_archive: ImageArchive) -> list:
        ...
        try:
            pe_client = internal_client_for(PolicyEngineClient,
                                            userId=self.account)
            vuln_report = pe_client.get_image_vulnerabilities(
                self.account, image_id)
            ...
            a = img_archive.add_artifact("vulnerabilities",
                                         source=None,
                                         data=data,
                                         metadata=metadata)
