...
from nextlinux_engine.common.models.policy_engine import Advisory, Artifact
from nextlinux_engine.common.models.policy_engine import (
    FeedGroupMetadata as APIFeedGroupMetadata, )
from nextlinux_engine.common.models.policy_engine import FeedMetadata as APIFeedMetadata
from nextlinux_engine.common.models.policy_engine import (
    FixedArtifact,
...
from nextlinux_engine.common.models.policy_engine import VulnerabilityMatch
...
from nextlinux_engine.db import ImageVulnerabilitiesReport as DBImageVulnerabilitiesReport
...
from nextlinux_engine.services.policy_engine.engine.vulns.feed_display_mapper import (
    FeedDisplayMapper, )
...
class VulnerabilitiesProvider(ABC):
    ...
    @abstractmethod
    def get_image_vulnerabilities(self,
                                  **kwargs) -> ImageVulnerabilitiesReport:
        """
    ...
    def get_sync_util_provider(
            self, sync_configs: Dict[str, SyncConfig]) -> SyncUtilProvider:
        """
    ...
    @abstractmethod
    def rescan_images_loaded_during_feed_sync(self, uuid: str,
    ...
    def _get_db_feeds(self) -> List[FeedMetadata]:
        """
        Returns all feeds excluding govulnersdb feed in detached state
        """
        feeds = get_all_feeds_detached()
        return list(
            filter(
                lambda feed: feed.name in self.__default_sync_config__.keys(),
                feeds))
    ...
    def get_feed_groups(self,
                        db_feed: FeedMetadata) -> List[APIFeedGroupMetadata]:
        """
    ...
    @abstractmethod
    def is_image_vulnerabilities_updated(self, image: Image, db_session,
    ...
    def update_feed_enabled_status(self, feed_name: str,
                                   enabled: bool) -> Optional[APIFeedMetadata]:
...
class LegacyProvider(VulnerabilitiesProvider):
    ...
    __default_sync_config__ = {
        ...
        "nvdv2":
        SyncConfig(enabled=True, url="https://ancho.re/v1/service/feeds"
                   ),  # for backwards selective sync compatibility
        "github":
        SyncConfig(enabled=False, url="https://ancho.re/v1/service/feeds"),
        "packages":
        SyncConfig(enabled=False, url="https://ancho.re/v1/service/feeds"),
    }
    ...
    def init_display_mapper(self) -> None:
        ...
        for feed_name in self.__default_sync_config__.keys():
            self._display_mapper.register(internal_name=feed_name,
    ...
    def load_image(self, image: Image, db_session, use_store=True):
        ...
        scanner.flush_and_recompute_vulnerabilities(image,
    ...
    def get_image_vulnerabilities_json(
        ...
        return self.get_image_vulnerabilities(image, db_session, vendor_only,
                                              force_refresh,
                                              use_store).to_json()
    ...
    def get_image_vulnerabilities(
        ...
        if force_refresh:
            logger.info("Forcing refresh of vulnerabilities for {}/{}".format(
                user_id, image_id))
            try:
                ...
                results.append(
                    VulnerabilityMatch(
                        vulnerability=VulnerabilityModel(
                            ...
                            cvss=
                            [],  # backwards compatibility - distro vulns didn't used to provide cvss
                        ),
                        artifact=Artifact(
                            name=vuln.pkg_name,
                    ...
                    results.append(
                        VulnerabilityMatch(
                            ...
                            fix=FixedArtifact(
                                ...
                                if vulnerability_cpe.get_fixed_in() else None,
                                advisories=[],
                            ),
    ...
    def get_images_by_vulnerability(
        ...
        ipm_query = (db_session.query(ImagePackageVulnerability).filter(
            ImagePackageVulnerability.vulnerability_id == vulnerability_id
        ).filter(ImagePackageVulnerability.pkg_user_id == account_id))
        icm_query = (db_session.query(ImageCpe, _cpe_cls).filter(
            _cpe_cls.vulnerability_id == vulnerability_id).filter(
                func.lower(ImageCpe.name) == _cpe_cls.name).filter(
                    ImageCpe.image_user_id == account_id).filter(
        ...
        if image_package_matches or image_cpe_matches or image_cpe_vlndb_matches:
            imageId_to_record = get_imageId_to_record(account_id,
            ...
            for cpe_matches in [image_cpe_matches, image_cpe_vlndb_matches]:
                ...
                logger.debug("IMAGECPEPKG TIME: {}".format(time.time() -
                                                           start))
    ...
    def get_sync_util_provider(
            self, sync_configs: Dict[str,
                                     SyncConfig]) -> LegacySyncUtilProvider:
        """
        Get a SyncUtilProvider.
        """
    ...
    def rescan_images_loaded_during_feed_sync(self, uuid: str,
        ...
        logger.info(
            "Rescanning images loaded between {} and {} (operation_id={})".
            format(from_time.isoformat(), to_time.isoformat(), uuid))
        count = 0
            ...
            for i in range(retry_max):
                try:
                    ...
                    try:
                        ...
                        if image_obj:
                            logger.info(
                                "Rescanning image {} post-vuln sync. (operation_id={})"
                                .format(img, uuid))
                            self.load_image(image_obj, db_session=db)
                            count += 1
                        else:
                            logger.warn(
                                "Failed to lookup image with tuple: {} (operation_id={})"
                                .format(str(img), uuid))
                    ...
                    logger.exception(
                        "Caught exception updating vulnerability scan results for image {}. Waiting and retrying (operation_id={})"
                        .format(img, uuid))
                    time.sleep(5)
    ...
    def is_image_vulnerabilities_updated(self, image: Image, db_session,
        ...
        feed_group_updated_list = [
            ...
            for feed in get_all_feeds(db_session) for group in feed.groups
        ]
        return (max(feed_group_updated_list) > since
...
class GovulnersProvider(VulnerabilitiesProvider):
    ...
    __default_sync_config__ = {
        ...
        SyncConfig(
            enabled=True,
            url=
            "https://toolbox-data.next-linux.systems/govulners/databases/listing.json",
        ),
        "packages":
        SyncConfig(enabled=False, url="https://ancho.re/v1/service/feeds"),
    }
    ...
    def init_display_mapper(self) -> None:
        ...
        Populate the display mapper for this provider.
        """
        self._display_mapper.register(internal_name="govulnersdb",
                                      display_name="vulnerabilities")
        self._display_mapper.register(internal_name="packages",
                                      display_name="packages")

    def load_image(self, image: Image, db_session, use_store=True):
        with timer("govulners provider load-image", log_level="info"):
    ...
    def get_image_vulnerabilities_json(
        ...
        with timer("govulners provider get-image-vulnerabilities-json",
                   log_level="info"):
            if force_refresh:
    ...
    def get_image_vulnerabilities(
        ...
        with timer("govulners provider get-image-vulnerabilities",
                   log_level="info"):
            if force_refresh:
                report = self._create_new_report(image, db_session, use_store)
            else:
    ...
    def _exclude_will_not_fix(matches: List[VulnerabilityMatch]):
        ...
        return (list(
            filter(
                lambda x: not (x.fix and isinstance(x.fix.will_not_fix, bool)
    ...
    def _create_new_report(
            ...
            image: Image,
            db_session,
            use_store: bool = True) -> ImageVulnerabilitiesReport:
        """
    ...
    def _try_load_report_from_store(
        ...
        try:
            ...
            if existing_report and report_status and report_status == Status.valid:
                metrics.counter_inc(
                    name="nextlinux_vulnerabilities_cache_hits")
                metrics.histogram_observe(
                    "nextlinux_vulnerabilities_cache_access_latency",
                    time.time() - timer2,
                    status="hit",
                )
                logger.info(
                ...
                metrics.counter_inc(
                    name="nextlinux_vulnerabilities_cache_misses")
                metrics.histogram_observe(
                    "nextlinux_vulnerabilities_cache_access_latency",
                    time.time() - timer2,
                    status="miss",
    ...
    def get_vulnerabilities(self, ids, affected_package,
                            affected_package_version, namespace, session):
        scanner = self.__scanner__()

        # Get vulnerability results from govulners db, through the govulners wrapper, via the scanner
    ...
    def get_images_by_vulnerability(
        ...
        #         },
        #     }
        with timer("govulners provider get-images-by-vulnerability",
                   log_level="info"):
            vulnerable_images = []
            ...
            db_records = (
                db_session.query(DBImageVulnerabilitiesReport).filter(
                    DBImageVulnerabilitiesReport.account_id ==
                    account_id).filter(
            ...
            image_to_record = get_imageId_to_record(account_id,
                                                    dbsession=db_session)
        ...
        for match in matches:
            ...
            if match.vulnerability.vulnerability_id == vulnerability_id:
                ...
                if severity_filter and severity_filter != match.vulnerability.severity:
                    continue
                if (namespace_filter and namespace_filter
                        != match.vulnerability.feed_group):
                    continue
                if (affected_package_filter
                        and affected_package_filter != match.artifact.name):
                    continue
                if (isinstance(vendor_only, bool)
                        and vendor_only  # true means vendor may fix
                        and
                        match.fix.will_not_fix  # true means vendor won't fix
                    ):
                    continue
    ...
    def get_sync_util_provider(
            self,
            sync_configs: Dict[str,
                               SyncConfig]) -> GovulnersDBSyncUtilProvider:
        """
        Get a SyncUtilProvider.
        """
    ...
    def rescan_images_loaded_during_feed_sync(self, uuid: str,
                                              ...
                                              to_time: datetime.datetime):
        """
        This is a no-op for govulners provider since vulnerability matches are not computed the same way as the legacy provider.
        govulners-db feed sync will refresh the matches for all images in the system
        """
        pass
            ...
            with session_scope() as session:
                try:
                    ...
                    for raw_group in active_db.groups:
                        groups.append(
                            APIFeedGroupMetadata.from_json(raw_group))
    ...
    def update_feed_group_counts(self) -> None:
        """
        Counts on govulnersdb are static so no need to update
        """
        return
    ...
    def is_image_vulnerabilities_updated(self, image: Image, db_session,
        ...
        It does not intentionally use the same mechanism as the legacy provider i.e. govulners-db feed last_sync timestamp,
        as that's not an accurate measure. The last_sync timestamp reflects when a sync was performed,
        not if a new govulners-db was synced. The more accurate approach is to compare the input with the report generation
        timestamp
        """
        # initialize store manager first
        store_manager = self.__store__(image)
    ...
    def delete_image_vulnerabilities(self, image: Image, db_session):
        ImageVulnerabilitiesStore(image_object=image).delete_all(
            session=db_session)
...
def set_provider():
    ...
    provider_class = next(
        (item for item in PROVIDER_CLASSES
         if item.__config__name__ == provider_name),
        None,
    )

    if not provider_class:
        raise ValueError(
            'No vulnerabilities provider implementation found for configured provider %s. Supported providers are "govulners" or "legacy"',
            provider_name,
        )

    PROVIDER = provider_class()
    logger.info("Initialized vulnerabilities provider: %s",
