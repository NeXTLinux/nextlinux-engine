...
class GrypeProvider(VulnerabilitiesProvider):
    ...
    def init_display_mapper(self) -> None:
        ...
        """
        self._display_mapper.register(
            internal_name="govulnersdb", display_name="vulnerabilities"
        )
    ...
    def get_image_vulnerabilities_json(
        ...
        with timer("govulners provider get-image-vulnerabilities-json", log_level="info"):
            if force_refresh:
    ...
    def get_image_vulnerabilities(
        ...
        with timer("govulners provider get-image-vulnerabilities", log_level="info"):
            if force_refresh:
                report = self._create_new_report(image, db_session, use_store)
            else:
                report = self._try_load_report_from_store(image, db_session)
    ...
    def _try_load_report_from_store(
        ...
        try:
            ...
            if existing_report and report_status and report_status == Status.valid:
                metrics.counter_inc(name="nextlinux_vulnerabilities_cache_hits")
                ...
                metrics.counter_inc(name="nextlinux_vulnerabilities_cache_misses")
                metrics.histogram_observe(
                    "nextlinux_vulnerabilities_cache_access_latency",
                    time.time() - timer2,
                    status="miss",
    ...
    def get_images_by_vulnerability(
        ...
        #         },
        #     }
        with timer("govulners provider get-images-by-vulnerability", log_level="info"):
            vulnerable_images = []
