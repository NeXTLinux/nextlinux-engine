...
class VerifyTrigger(BaseTrigger):
    ...
    directories = CommaDelimitedStringListParameter(
        ...
        description="List of directories to limit checks so as to avoid checks on all dir.",
        is_required=False,
        sort_order=2,
    )
        ...
        for pkg in pkgs:
            ...
            for pkg_db_record in records:
                status = self._diff_pkg_meta_and_file(
                    pkg_db_record, extracted_files_json.get(pkg_db_record.file_path)
                )

                if status and (check is None or status == check):
                    self._fire(
                        msg="VERIFY check against package db for package '{}' failed on entry '{}' with status: '{}'".format(
                            pkg_name, pkg_db_record.file_path, status.value
                        )
                    )
    ...
    def _diff_pkg_meta_and_file(cls, meta_db_entry, fs_entry):
        ...
        # This is unexpected
        if (fs_entry and not meta_db_entry) or fs_entry.get(
            "name"
        ) != meta_db_entry.file_path:
            return False
