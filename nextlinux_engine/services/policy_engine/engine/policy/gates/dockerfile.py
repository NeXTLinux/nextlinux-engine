...
class ParameterizedDockerfileModeBaseTrigger(BaseTrigger):
    ...
    actual_dockerfile_only = BooleanStringParameter(
        ...
        example_str="true",
        description="Only evaluate against a user-provided dockerfile, skip evaluation on inferred/guessed dockerfiles. Default is False.",
        is_required=False,
    )
...
class EffectiveUserTrigger(DockerfileModeCheckedBaseTrigger):
    ...
    user = CommaDelimitedStringListParameter(
        ...
        description="User names to check against as the effective user (last user entry) in the images history.",
        is_required=True,
    ...
    def _evaluate(self, image_obj, context):
        ...
        if is_allowed and user not in rule_users:
            self._fire(
                msg="User {} found as effective user, which is not on the allowed list".format(
                    user
                )
            )
        elif not is_allowed and user in rule_users:
            self._fire(
                msg="User {} found as effective user, which is explicity not allowed".format(
                    user
                )
            )
...
class InstructionCheckTrigger(ParameterizedDockerfileModeBaseTrigger):
    ...
    _operations_requiring_check_val = ["=", "!=", "like", "not_like", "in", "not_in"]
    ...
    def _evaluate(self, image_obj, context):
        ...
        for directive_name, lines in [x for x in list(df.items()) if x[0] == directive]:
            for l in lines:
                ...
                if operation(l, check_value):
                    self._fire(
                        msg="Dockerfile directive '{}' check '{}' matched against '{}' for line '{}'".format(
                            directive_name,
                            condition,
        ...
        if condition == "not_exists" and directive not in upper_keys:
            self._fire(
                msg="Dockerfile directive '{}' not found, matching condition '{}' check".format(
                    directive, condition
                )
            )
                        ...
                        for p in blacklisted_ports:
                            if p in iexpose:
                                self._fire(
                                    ...
                                    + p
                                    + ") which is in policy file DENIEDPORTS list"
                                )
                            elif p + "/tcp" in iexpose:
                                self._fire(
                                    msg="Dockerfile exposes port ("
                                    + p
                                    + "/tcp) which is in policy file DENIEDPORTS list"
                                )
                    ...
                    if 0 in whitelisted_ports and len(iexpose) > 0:
                        ...
                        for ip in iexpose:
                            self._fire(
                                msg="Dockerfile exposes port ("
                                + ip
                                + ") which is not in policy file ALLOWEDPORTS list"
                            )
        return
                ...
                if line and not line.startswith("#"):
                    ...
                    if patt:
                        ...
                        if linebuf:
                            ...
                            context.data["prepared_dockerfile"][directive].append(
                                linebuf
                            )
                            linebuf = ""
                else:
