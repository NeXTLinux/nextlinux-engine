...
class WhitelistAwarePolicyDecider(object):
    ...
    def decide(cls, decisions):
        candidate_actions = [
            x.action for x in
            [d for d in decisions if not getattr(d, "is_whitelisted", False)]
        ]
...
class PolicyRuleDecision(object):
    ...
    def action(self):
        ...
        """
        if self.match and not (hasattr(self.match, "is_whitelisted")
                               and self.match.is_whitelisted):
...
class ErrorMatch(TriggerMatch):
    ...
    class EmptyTrigger(BaseTrigger):
        ...
        def __init__(self, parent_gate_cls, msg=None):
            ...
            self.msg = ("Trigger implementation not found, this is placeholder"
                        if not msg else msg)

    def __init__(self, trigger, match_instance_id=None, msg=None):
        self.trigger = (trigger if trigger else ErrorMatch.EmptyTrigger(
            ErrorMatch.EmptyGate))
...
class PolicyRuleFailure(PolicyRuleDecision):
    ...
    def __init__(self, trigger_match, policy_rule, failure_msg, failure_cause):
        ...
        """
        self.match = (trigger_match
                      if trigger_match else ErrorMatch(None, msg=failure_msg))
        self.policy_rule = policy_rule
    ...
    def json(self):
        return {
            ...
            "error_cause":
            self.cause.message
            if hasattr(self.cause, "message") else str(self.cause),
        }
...
class PolicyDecision(object):
    ...
    def json(self):
        return {
            "policy":
            self.evaluated_policy.json() if self.evaluated_policy else None,
            "decisions":
            [r.json() for r in self.decisions] if self.decisions else None,
...
class BundleDecision(object):
    ...
    def __init__(self,
        ...
        self.policy_decisions = (policy_decisions if policy_decisions else
                                 [FallThruPolicyDecision()])
    ...
    def json(self):
        return {
            ...
            ] if self.policy_decisions else None,
            "policy_final_action":
            self.final_policy_decision.name,
            "matched_whitelisted_image_rule":
            self.whitelisted_image.json() if self.whitelisted_image else None,
            "matched_blacklisted_image_rule":
            self.blacklisted_image.json() if self.blacklisted_image else None,
            "final_action":
            self.final_decision.name,
            "reason":
            self.reason,
        }
...
class BundleExecution(object):
    ...
    def json(self):
        return {
            "bundle":
            self.executed_bundle.json() if self.executed_bundle else None,
            "mapping":
            self.executed_mapping.json() if self.executed_mapping else None,
            "image_id":
            self.image_id,
            "tag":
            self.tag,
            "bundle_decision":
            self.bundle_decision.json() if self.bundle_decision else None,
            "warnings":
            [str(x) for x in self.warnings] if self.warnings else None,
    ...
    def _row_json(self, policy_rule_decision):
        ...
        return [
            ...
            policy_rule_decision.match.whitelisted_json() if hasattr(
                policy_rule_decision.match, "whitelisted_json") else False,
    ...
    def as_table_json(self):
        ...
        table = {
            self.image_id: {
                "result": {
                    "header":
                    self.CLI_COMPATIBLE_HEADER_SET,
                    "row_count":
                    len(rows),
                    "rows":
                    rows,
                    "final_action":
                    self.bundle_decision.final_policy_decision.name,
                },
            },
            "policy_data": [],
            "whitelist_data": [],
            "policy_name":
            "",  # Use empty string here instead of 'null' to make more parser friendly
            "whitelist_names": [],
        }
    ...
    def __init__(self, parent, policy_json=None):
        ...
        try:
            ...
            raise InvalidGateAction(
                ...
                action=action,
                valid_actions=[
                    x for x in list(GateAction.__dict__.keys())
...
def policy_rule_factory(policy_obj, policy_json, strict_validation=True):
    ...
    if strict_validation:
        if rule.gate_cls.__lifecycle_state__ == LifecycleStates.eol:
            raise EndOfLifedError(rule.gate_name,
                                  superceded=rule.gate_cls.__superceded_by__)
...
class ExecutablePolicyRule(PolicyRule):
    ...
    def execute(self, image_obj, exec_context):
        ...
        try:
            ...
            elif (self.configured_trigger.__lifecycle_state__ ==
            ...
            logger.exception("Error executing trigger {} on image {}".format(
    ...
    def execute(self, image_obj, context):
        ...
        for gate, policy_rules in list(self.gates.items()):
            ...
            for rule in policy_rules:
                errs, matches = rule.execute(image_obj=image_obj,
...
class MappingRule(object):
    ...
    def json(self):
        if self.raw:
            ...
            return {
                ...
                "image": {
                    "type":
                    self.image_match_type,
                    "value":
                    self.image_tag if self.image_tag else
                    self.image_digest if self.image_digest else
                    self.image_id if self.image_id else None,
                },
            }
    ...
    def matches(self, image_obj, tag):
        ...
        if not tag:
            raise ValueError(
                "Tag cannot be empty or null for matching evaluation")
        else:
            match_target = parse_dockerimage_string(tag, strict=False)

        # Must match registry and repo first
        if not (self._registry_match(match_target.get("registry"))
...
class PolicyMappingRule(MappingRule):
    ...
    def __init__(self, rule_json=None):
        ...
        if rule_json.get("policy_id"):
            ...
            raise ValidationError(
                "No policy_id or policy_ids property found for mapping rule: {}"
                .format(rule_json))
...
class ExecutableMapping(object):
    ...
    def execute(self, image_obj, tag):
        ...
        result = [
            y for y in self.mapping_rules if y.matches(image_obj, target_tag)
        ]
...
class WhitelistedTriggerMatch(TriggerMatch):
    ...
    def __init__(self, trigger_match, matched_whitelist_item):
        super(WhitelistedTriggerMatch,
              self).__init__(trigger_match.trigger, trigger_match.id,
                             trigger_match.msg)
...
class ExecutableWhitelistItem(object):
    ...
    def execute(self, trigger_match):
        ...
        if hasattr(trigger_match,
                   "is_whitelisted") and trigger_match.is_whitelisted():
            return trigger_match
    ...
    def matches(self, fired_trigger_obj):
        ...
        return (self.gate
                == fired_trigger_obj.trigger.gate_cls.__gate_name__.lower() and
...
class HybridTriggerIdKeyedItemIndex(IWhitelistItemIndex):
    ...
    def add(self, item):
        ...
        if gate_name not in self.gate_keys:
            self.gate_keys[
                gate_name] = HybridTriggerIdKeyedItemIndex.GateItemIndex(
...
class ExecutableBundle(VersionedEntityMixin):
        ...
        try:
            ...
            for rule in self.mapping.mapping_rules:
                try:
                    ...
                    policies = {
                        policy_id: [
                            x for x in self.raw.get("policies", [])
                            if x["id"] == policy_id
                        ]
                ...
                for wl in rule.whitelist_ids:
                    try:
                        whitelist = [
                            x for x in self.raw.get("whitelists", [])
                            if x["id"] == wl
                        ]
                        if not whitelist:
                            raise ReferencedObjectNotFoundError(
    ...
    def _process_mapping(self, bundle_exec, image_object, tag):
        ...
        try:
            if self.mapping:
                bundle_exec.executed_mapping = self.mapping.execute(
                    image_object, tag)
            else:
        ...
        try:
            ...
            if evaluated_policies:
                for evaluated_policy in evaluated_policies:
                    ...
                    if errors:
                        logger.warn(
                            "Evaluation encountered errors/warnings: {}".
                            format(errors))
                        bundle_exec.errors += errors

                    # Send thru the whitelist handlers
                    for wl in bundle_exec.executed_mapping.whitelist_ids:
                        policy_decision.decisions = self.whitelists[
                            wl].execute(policy_decision.decisions)
    ...
    def execute(self, image_object, tag, context):
        ...
        bundle_exec = self._process_mapping_result(bundle_exec, image_object,
                                                   tag, context)
...
def build_empty_error_execution(image_obj,
                                tag,
                                bundle,
                                errors=None,
                                warnings=None):
    ...
    b.bundle_decision = BundleDecision(
        policy_decisions=[FailurePolicyDecision()])
    b.errors = errors
    b.warnings = warnings
    return b
