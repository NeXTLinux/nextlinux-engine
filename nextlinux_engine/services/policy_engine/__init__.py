...
def init_feed_registry():
    ...
    for cls_tuple in [
        ...
        logger.info("Registering feed handler {}".format(
            cls_tuple[0].__feed_name__))
        feed_registry.register(cls_tuple[0],
                               is_vulnerability_feed=cls_tuple[1])
...
class PolicyEngineService(ApiService):
    ...
    def get_cpe_vulnerabilities(self, image, nvd_cls, cpe_cls):
        with timer("Image vulnerability cpe lookups", log_level="debug"):
            return image.cpe_vulnerabilities(_nvd_cls=nvd_cls,
                                             _cpe_cls=cpe_cls)
