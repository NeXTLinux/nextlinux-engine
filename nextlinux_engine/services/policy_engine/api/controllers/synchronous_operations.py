...
from nextlinux_engine import apis, utils
...
from nextlinux_engine.clients.services import catalog, internal_client_for
from nextlinux_engine.common.helpers import make_response_error

# API models
from nextlinux_engine.common.models.policy_engine import GateSpec
from nextlinux_engine.common.models.policy_engine import Image as ImageMsg
...
class EvaluationCacheManager(object):
    ...
    def _should_evaluate(self, cache_entry: CachedPolicyEvaluation):
        if cache_entry is None:
            metrics.counter_inc(name="nextlinux_policy_evaluation_cache_misses_notfound")
        ...
        if cache_entry.bundle_digest == self.bundle_digest:
            ...
            if self._inputs_changed(cache_entry.last_modified):
                metrics.counter_inc(name="nextlinux_policy_evaluation_cache_misses_stale")
                return EvaluationCacheManager.CacheStatus.stale
            else:
                return EvaluationCacheManager.CacheStatus.valid
        else:
            metrics.counter_inc(name="nextlinux_policy_evaluation_cache_misses_invalid")
        ...
        if evaluation_cache_enabled:
            ...
            try:
                ...
                if cache_mgr is None:
                    ...
                    if cached_result:
                        metrics.counter_inc(name="nextlinux_policy_evaluation_cache_hits")
