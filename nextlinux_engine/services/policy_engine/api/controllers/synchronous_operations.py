...
evaluation_cache_enabled = (os.getenv(
    "NEXTLINUX_POLICY_ENGINE_EVALUATION_CACHE_ENABLED",
    "true").lower() == "true")
...
class ImageMessageMapper(object):
    ...
    def db_to_msg(self, db_obj):
        msg = ImageMsg()
        msg.last_modified = db_obj.last_modified.strftime(
            self.rfc3339_date_fmt)
...
def set_cache_status(status):
    ...
    if status.get("enabled") is not None and type(
            status.get("enabled")) == bool:
        evaluation_cache_enabled = status.get("enabled")
        return {"enabled": evaluation_cache_enabled}, 200
    else:
        return make_response_error(errmsg="Invalid request",
                                   in_httpcode=400), 400
...
def delete_image(user_id, image_id):
    ...
    db = get_session()
    try:
        log.info("Deleting image {}/{} and all associated resources".format(
            user_id, image_id))
        ...
        if img:
            ...
            #    db.delete(pkg_vuln)
            try:
                conn_timeout = ApiRequestContextProxy.get_service(
                ).configuration.get("catalog_client_conn_timeout",
                                    DEFAULT_CACHE_CONN_TIMEOUT)
                read_timeout = ApiRequestContextProxy.get_service(
                ).configuration.get("catalog_client_read_timeout",
                                    DEFAULT_CACHE_READ_TIMEOUT)
                mgr = EvaluationCacheManager(img, None, None, conn_timeout,
                                             read_timeout)
                mgr.flush()
            except Exception as ex:
                log.exception(
                    "Could not delete evaluations for image {}/{} in the cache. May be orphaned"
        ...
        log.exception("Error processing DELETE request for image {}/{}".format(
            user_id, image_id))
        db.rollback()
        return (
            make_response_error(
...
class EvaluationCacheManager(object):
    ...
    def __init__(
        ...
        self._catalog_client = internal_client_for(catalog.CatalogClient,
                                                   userId=self.image.user_id)
    ...
    def _digest_for_bundle(self):
        return hashlib.sha256(
            utils.ensure_bytes(json.dumps(self.bundle,
        ...
        if entry.is_archive_ref():
            ...
            while retry > 0:
                try:
                    ...
                    log.exception(
                        "Could not delete policy eval from cache, will retry. Bucket={}, Key={}"
                        .format(bucket, key))
                    retry -= 1
            else:
                log.error(
    ...
    def _lookup(self):
        ...
        session = get_session()
        return (session.query(CachedPolicyEvaluation).filter_by(
    ...
    def save(self, result):
        ...
        key = ("sha256:" + hashlib.sha256(
            utils.ensure_bytes(str(eval.key_tuple()))).hexdigest())
        with self._catalog_client.timeout_context(
                self._default_catalog_conn_timeout,
                self._default_catalog_read_timeout) as timeout_client:
            resp = timeout_client.put_document(self.__cache_bucket__, key,
                                               result)
        ...
        result_digest = (
            "sha256:" +
            hashlib.sha256(utils.ensure_bytes(str_result)).hexdigest())
    ...
    def flush(self):
        ...
        for entry in session.query(CachedPolicyEvaluation).filter_by(
            ...
            try:
                ...
                log.exception(
                    "Could not delete eval cache entry: {}".format(entry))
...
def get_image_vulnerabilities(user_id,
    ...
    try:
        ...
        if not img:
            ...
            with timer("Image vulnerability primary lookup",
                       log_level="debug"):
        ...
        with timer("Image vulnerability nvd metadata merge",
                   log_level="debug"):
            vulns = merge_nvd_metadata_image_packages(db, vulns, _nvd_cls,
                                                      _cpe_cls)
        ...
        with timer("Image vulnerability output formatting", log_level="debug"):
            for vuln, nvd_records in vulns:
                ...
                if vuln.pkg_name != vuln.package.fullversion:
                    pkg_final = "{}-{}".format(vuln.pkg_name,
        ...
        try:
            ...
            with timer("Image vulnerabilities cpe matches", log_level="debug"):
                all_cpe_matches = (ApiRequestContextProxy.get_service(
                ).get_cpe_vulnerabilities(img, _nvd_cls, _cpe_cls))
                ...
                for image_cpe, vulnerability_cpe in all_cpe_matches:
                    ...
                    cpe_vuln_el = {
                        ...
                        "severity":
                        vulnerability_cpe.parent.severity,
                        "link":
                        link,
                        "pkg_type":
                        image_cpe.pkg_type,
                        ...
                        image_cpe.version,
                        "cpe":
                        image_cpe.get_cpestring(),
                        "cpe23":
                        image_cpe.get_cpe23string(),
                        "feed_name":
                        vulnerability_cpe.feed_name,
                        "feed_namespace":
                        vulnerability_cpe.namespace_name,
                        "nvd_data":
                        vulnerability_cpe.parent.get_cvss_data_nvd(),
                        "vendor_data":
                        vulnerability_cpe.parent.get_cvss_data_vendor(),
                        "fixed_in":
                        vulnerability_cpe.get_fixed_in(),
                    }
                    cpe_hash = hashlib.sha256(
                        utils.ensure_bytes(
                            json.dumps(cpe_vuln_el))).hexdigest()
...
def _get_imageId_to_record(userId, dbsession=None):
    ...
    for x in imagetags:
        ...
        if x["tag_detected_at"]:
            tag_detected_at = (datetime.datetime.utcfromtimestamp(
                float(int(x["tag_detected_at"]))).isoformat() + "Z")
        else:
            tag_detected_at = 0
        ...
        if x["imageId"] not in imageId_to_record:
            if x["analyzed_at"]:
                analyzed_at = (datetime.datetime.utcfromtimestamp(
                    float(int(x["analyzed_at"]))).isoformat() + "Z")
            else:
                analyzed_at = 0
...
def query_images_by_package(dbsession, request_inputs):
    ...
    pkg_hash = {}
    try:
        ipm_query = (dbsession.query(ImagePackage).filter(
            ImagePackage.name == pkg_name).filter(
                ImagePackage.image_user_id == userId))
        cpm_query = (dbsession.query(ImageCpe).filter(
            func.lower(ImageCpe.name) == func.lower(pkg_name)).filter(
        ...
        if image_package_matches or cpe_package_matches:
            imageId_to_record = _get_imageId_to_record(userId,
                                                       dbsession=dbsession)
...
def check_no_advisory(image):
    phash = hashlib.sha256(
        json.dumps([
            image.pkg_name, image.pkg_version,
            image.vulnerability_namespace_name
...
def query_images_by_vulnerability(dbsession, request_inputs):
    ...
    try:
        ...
        ipm_query = (dbsession.query(ImagePackageVulnerability).filter(
            ImagePackageVulnerability.vulnerability_id == id).filter(
                ImagePackageVulnerability.pkg_user_id == userId))
        icm_query = (dbsession.query(
            ImageCpe, _cpe_cls).filter(_cpe_cls.vulnerability_id == id).filter(
                func.lower(ImageCpe.name) == _cpe_cls.name).filter(
                    ImageCpe.image_user_id == userId).filter(
...
def query_vulnerabilities(dbsession, ids, package_name_filter,
                          package_version_filter, namespace):
    """
    ...
    try:
        ...
        qry = (dbsession.query(_nvd_cls).options(
        ...
        nvd_vulnerabilities.extend(
            dbsession.query(VulnDBMetadata).options(
        ...
        if namespace == ["nvdv2:cves"]:
            ...
            qry = (dbsession.query(Vulnerability).options(
...
def list_artifacts(user_id, image_id, artifact_type):
    db = get_session()
    try:
        log.info("Getting retrieved files from image {}/{}".format(
            user_id, image_id))
        img = db.query(Image).get((image_id, user_id))
        if img:
...
def retrieved_file_to_mgs(artifact_record):
    ...
    return {
        "path":
        artifact_record.artifact_key,
        "b64_content":
        ensure_str(base64.encodebytes(artifact_record.binary_value)),
    }
