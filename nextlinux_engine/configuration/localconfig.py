...
DEFAULT_CONFIG = {
    "service_dir": os.path.join(
        "{}".format(os.getenv("HOME", "/tmp/nextlinuxtmp")), ".nextlinux_engine"
    ),
    "tmp_dir": "/tmp",
    "log_level": "INFO",
...
POLICY_BUNDLE_SOURCE_DIRS = [
    os.path.join(resource_filename("nextlinux_engine", "conf/bundles/"))
]
...
def load_filepath_to_config(key, fname, src_dir=None):
    ...
    try:
        ...
        if src_dir == None:
            src_dir = os.path.join(resource_filename("nextlinux_engine", "conf/"))
