...
USER_MOD_PROTECTED_ACCOUNT_NAMES = [
    GLOBAL_RESOURCE_DOMAIN, SYSTEM_ACCOUNT_NAME
]
        ...
        if policy_bundles_dir is not None:
            ...
            for file_name in os.listdir(src_dir):
                try:
                    ...
                    logger.warn(
                        "Policy bundle {} not found, unable to load. Exception: {}"
                        .format(file_name, e))
...
def read_config(configfile=None):
    ...
    if not configfile or not os.path.exists(configfile):
        raise Exception("no config file (" + str(configfile) +
                        ") can be found to load")
    else:
        try:
...
def validate_config(config, validate_params=None):
    ...
    try:
        ...
        if ("user_authentication" in validate_params
...
def validate_user_auth_config(config):
    ...
    if not config.get("user_authentication"):
        raise Exception(
            "user_authentication property in configuration must be present")
    else:
...
def validate_key_config(config, required=False):
    ...
    if not (kconf.get("secret") or
            (kconf.get("public_key_path") and kconf.get("public_key_path"))):
        raise Exception(
