    ...
    if config:
        if config.get("private_key_path"):
            ...
            try:
                ...
                raise Exception(
                    "Could not load private key file from path: {}. Error: {}".
                    format(priv_keypath, e))
        ...
        if config.get("public_key_path"):
            ...
            try:
                ...
                raise Exception(
                    "Could not load public key file from path: {}. Error: {}".
                    format(pub_keypath, e))
...
class TokenIssuer(object):
    ...
    def generate_token(self, subject):
        ...
        if not self.signing_key:
            ...
            expiration_ts = ts + datetime.timedelta(
...
class JwtTokenManager(object):
    ...
    def __init__(self, oauth_config, keys_config):
        ...
        self.issuers = {
            name:
            TokenIssuer(key, "RS256", expiration) if name != "secret" else
            TokenIssuer(key, "HS256", expiration, issuer=NEXTLINUX_ISSUER)
            for name, key in self.keys.items()
            if name in ["private", "secret"]
        }
        self.verifiers = {
            name:
            TokenVerifier(key, "RS256", issuers=[NEXTLINUX_ISSUER])
            if name != "secret" else TokenVerifier(key, "HS256")
            for name, key in self.keys.items() if name in ["public", "secret"]
        }
    ...
    def generate_token(self,
        ...
        if not with_key_name:
            ...
            tok, exp = self.issuers[self._default_private].generate_token(
                user_uuid)
        else:
...
def oauth_config_loader(config: dict):
    ...
    return config.get("user_authentication",
                      {}).get("oauth"), config.get("keys")
