    ...
    for d in list(ret["dirs"].keys()):
        if not os.path.isdir(ret["dirs"][d]):
            try:
                ...
                print("ERROR: cannot find/create input dir '" +
                      ret["dirs"][d] + "'")
                raise err
...
def run_tarfile_member_function(tarfilename,
                                *args,
                                member_regexp=None,
                                func=_default_member_function,
                                **kwargs):
        ...
        for member in list(memberhash.values()):
            if not memberpatt or memberpatt.match(member.name):
                if ret.get(member.name):
                    print(
                        "WARN: duplicate member name when preparing return from run_tarfile_member_function() - {}"
                        .format(member.name))
...
def get_distro_from_squashtar(squashtar, unpackdir=None):
    if unpackdir and os.path.exists(
            os.path.join(unpackdir, "analyzer_meta.json")):
    ...
    with tarfile.open(squashtar, mode="r", format=tarfile.PAX_FORMAT) as tfl:
        ...
        metamap = {
            "os-release":
            _search_tarfilenames_for_file(tarfilenames, "etc/os-release"),
            "system-release-cpe":
            _search_tarfilenames_for_file(tarfilenames,
                                          "etc/system-release-cpe"),
            "redhat-release":
            _search_tarfilenames_for_file(tarfilenames, "etc/redhat-release"),
            "busybox":
            _search_tarfilenames_for_file(tarfilenames, "bin/busybox"),
            "debian_version":
            _search_tarfilenames_for_file(tarfilenames, "etc/debian_version"),
        }
        ...
        if not success and metamap["busybox"] in tarfilenames:
            try:
                ...
                try:
                    with tfl.extractfile(tfl.getmember(
                            metamap["busybox"])) as FH:
                        for line in FH.readlines():
                            patt = re.match(rb".*BusyBox (v[\d|\.]+) \(.*",
                                            line)
                            if patt:
                                meta[
        ...
        if (meta["DISTRO"] == "debian" and not meta["DISTROVERS"]
...
def grouper(inlist, chunksize):
    return (inlist[pos:pos + chunksize]
            for pos in range(0, len(inlist), chunksize))
...
def _get_extractable_member(tfl,
                            member,
                            deref_symlink=False,
                            alltfiles={},
                            memberhash={}):
    ...
    if deref_symlink and member.issym():
        ...
        while not done and count < max_links:
            ...
            if nmember.linkname[1:] in alltfiles:
                ...
                if nmember.linkname in alltfiles:
                    ...
                    normpath = os.path.normpath(
                        os.path.join(os.path.dirname(nmember.name),
                                     nmember.linkname))
            ...
            if not newmember:
                print(
                    "skipping file: looking for symlink destination for symlink file {} -> {}"
        ...
        while not done and count < max_links:
            try:
                ...
                print(
                    "WARN: exception while looking for hardlink destination for hardlink file {} - exception: {}"
...
def _checksum_member_function(tfl,
                              member,
                              csums=["sha256", "md5"],
                              memberhash={}):
    ret = {}
    ...
    if member.isreg():
        ...
        if not memberhash:
            memberhash = get_memberhash(tfl)
        extractable_member = _get_extractable_member(tfl,
                                                     member,
...
def _hints_to_binary(pkg):
    ...
    for inp in [pkg_files]:
        if type(inp) is not list:
            raise Exception(
                "bad hints record ({}), versions, licenses, origins, and files if specified must be list types"
                .format(pkg_name))
    el = {
        "name": pkg_name,
        "version": pkg_version,
...
def get_hintsfile(unpackdir=None, squashtar=None):
    ...
    def read_hints(path):
        ...
        with open(path, "r") as FH:
            try:
                ...
                print(
                    "WARN: hintsfile found unpacked, but cannot be read - exception: {}"
                    .format(err))
                return {}
        ...
        with tarfile.open(squashtar, mode="r",
            ...
            if hints_member:
                try:
                    ...
                    print(
                        "WARN: hintsfile found in squashtar, but cannot be read - exception: {}"
                        .format(err))
                    ret = {}
            else:
                ret = {}
    ...
    for pkg_type in ret.get("packages", []):
        if not all((pkg_type.get("name"), pkg_type.get("version"),
                    pkg_type.get("type"))):
            logger.error(
                "bad hints record, all hints records must supply at least a name, version and type"
            )
    ...
    for key in b:
        if key in a:
            if isinstance(a[key], dict) and isinstance(b[key], dict):
                ...
                raise Exception("dict merge conflict at %s" %
                                ".".join(path + [str(key)]))
        else:
            a[key] = b[key]
    return a
...
def content_hints(unpackdir):
    ...
    for package in hints.get("packages", []):
        ...
        if not all((package.get("name"), package.get("version"),
                    package.get("type"))):
            continue
