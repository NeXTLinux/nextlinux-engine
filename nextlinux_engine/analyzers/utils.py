                ...
                try:
                    with tfl.extractfile(tfl.getmember(metamap["busybox"])) as FH:
                        for line in FH.readlines():
                            ...
                            if patt:
                                meta["DISTROVERS"] = nextlinux_engine.utils.ensure_str(
                                    patt.group(1)
                                )
                except Exception as err:
                    meta["DISTROVERS"] = "0"
...
def _hints_to_binary(pkg):
    pkg_type = nextlinux_engine.utils.ensure_str(pkg.get("type", "binary")).lower()
    pkg_name = nextlinux_engine.utils.ensure_str(pkg.get("name", ""))
    pkg_version = nextlinux_engine.utils.ensure_str(pkg.get("version", ""))
    pkg_location = nextlinux_engine.utils.ensure_str(
...
def get_hintsfile(unpackdir=None, squashtar=None):
    ...
    if ret and not os.path.exists(os.path.join(unpackdir, "nextlinux_hints.json")):
