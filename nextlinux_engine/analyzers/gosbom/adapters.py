...
def _filter_artifacts(artifacts, relationships):
    ...
    def filter_fn(artifact):
        ...
        if _filter_relationships(relationships,
...
def _convert_gosbom_to_engine(gosbom_output: dict,
                              enable_package_filtering: bool) -> dict:
    """
    ...
    for artifact in artifacts:
        ...
        if artifact["type"] not in modules_by_artifact_type:
            logger.warn(
                "Handler for artifact type {} not available. Skipping package {}."
                .format(artifact["type"], artifact["name"]))
            continue
...
class FilteringEngineAdapter(IdentityAdapter):
    ...
    def __init__(self,
                 gosbom_output: dict,
                 enable_package_filtering: bool = True):
        super().__init__(gosbom_output)
        self.enable_package_filtering = enable_package_filtering

    def convert(self):
        return _convert_gosbom_to_engine(self.gosbom_output,
                                         self.enable_package_filtering)
