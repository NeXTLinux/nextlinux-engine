...
- **Analyze** the image by running a set of Nextlinux analyzers over the image content to extract and classify as much metadata as possible
...
There are, generally speaking, two different ways to use Nextlinux, within its single API:
...
- Watch Mode - Use the APIs to configure Nextlinux Engine to poll specific registries and repositories/tags to watch for new images added and automatically pull and evaluate them, emitting notifications when a given tag's vulnerability or policy evaluation state changes

With these two modes of operation, integration into CI/CD with Interative Mode or less intrusive watching of production image repositories with Watch Mode, Nextlinux can be easily integrated into most environments and processes.